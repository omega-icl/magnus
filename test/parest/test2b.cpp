...
// The problem of calibrating a dynamic model to describe dissolved oxygen
// concentration in a flask after inoculating biomass
////////////////////////////////////////////////////////////////////////
int
main()
////////////////////////////////////////////////////////////////////////
{
  ...
  mc::FFVar& mu = P[0];
  P[0].set(&DAG, "mu");  // max growth rate
  mc::FFVar& beta = P[1];
  P[1].set(&DAG, "beta");  // decay rate
  mc::FFVar& B0 = P[2];
  P[2].set(&DAG, "X(0)");  // initial biomass

  std::vector<mc::FFVar> C(NC);  // Constants
  mc::FFVar& f = C[0];
  C[0].set(&DAG, "f");  // biodegradable fraction
  mc::FFVar& y = C[1];
  C[1].set(&DAG, "Y");  // yield
  mc::FFVar& S0 = C[2];
  C[2].set(&DAG, "S0");  // initial substrate
  mc::FFVar& D0 = C[3];
  C[3].set(&DAG, "D0");  // initial dissolved oxygen

  std::vector<mc::FFVar> X(NX);  // States & state sensitivities
  mc::FFVar& B = X[0];
  X[0].set(&DAG, "X");  // biomass concentration
  mc::FFVar& S = X[1];
  X[1].set(&DAG, "S");  // substrate concentration
  mc::FFVar& D = X[2];
  X[2].set(&DAG, "D");  // dissolved oxygen concentration
  ...
  for (unsigned int j = 0; j < NY * (NS + 1); j++)
    Y[j] = OpODE(j, NP, P.data(), NC, C.data(), &IVP);
  // std::cout << DAG;
  ...
  std::vector<mc::PAREST::Experiment> Data{
      {{}, {{0, {{10.0}, 1e-2}},  {1, {{9.80}, 1e-2}},  {2, {{9.70}, 1e-2}},
            {3, {{9.30}, 1e-2}},  {4, {{8.80}, 1e-2}},  {5, {{8.55}, 1e-2}},
            {6, {{8.50}, 1e-2}},  {7, {{8.35}, 1e-2}},  {8, {{8.40}, 1e-2}},
            {9, {{8.30}, 1e-2}},  {10, {{8.20}, 1e-2}}, {11, {{8.20}, 1e-2}},
            {12, {{8.05}, 1e-2}}, {13, {{8.10}, 1e-2}}, {14, {{8.00}, 1e-2}},
            {15, {{7.90}, 1e-2}}, {16, {{8.00}, 1e-2}}, {17, {{7.90}, 1e-2}},
            {18, {{7.85}, 1e-2}}, {19, {{7.80}, 1e-2}}, {20, {{7.80}, 1e-2}},
            {21, {{7.75}, 1e-2}}, {22, {{7.70}, 1e-2}}, {23, {{7.55}, 1e-2}},
            {24, {{7.60}, 1e-2}}, {25, {{7.60}, 1e-2}}, {26, {{7.50}, 1e-2}},
            {27, {{7.55}, 1e-2}}, {28, {{7.50}, 1e-2}}, {29, {{7.45}, 1e-2}},
            {30, {{7.50}, 1e-2}}, {31, {{7.40}, 1e-2}}, {32, {{7.35}, 1e-2}},
            {33, {{7.30}, 1e-2}}, {34, {{7.35}, 1e-2}}, {35, {{7.30}, 1e-2}},
            {36, {{7.30}, 1e-2}}, {37, {{7.25}, 1e-2}}, {38, {{7.30}, 1e-2}},
            {39, {{7.20}, 1e-2}}, {40, {{7.20}, 1e-2}}}}};
  ...
  std::vector<mc::PAREST::Experiment> Data2{{{6.0},
                                             {
                                                 {0, {{9.99}, 1e-2}},
                                                 {1, {{9.96}, 1e-2}},
                                                 {2, {{9.93}, 1e-2}},
  ...
  for (unsigned int j = 0; j < NY * NS2; j++)
    Y2[j] = OpODE(j, NP, P.data(), NC, C2.data(), &IVP);
  std::cout << DAG;
