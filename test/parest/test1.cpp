...
// The problem of finding an optimal design to discriminate between a cubic
// polynomial model and a linear model was considered by Dette & Titoff
// (https://doi.org/10.1214/08-AOS635)
////////////////////////////////////////////////////////////////////////
int
main()
////////////////////////////////////////////////////////////////////////
{
  ...
  std::list<std::pair<double, std::vector<double>>> Campaign{
      {1, {-1e0}},  {1, {-9e-1}}, {1, {-8e-1}}, {1, {-7e-1}}, {1, {-6e-1}},
      {1, {-5e-1}}, {1, {-4e-1}}, {1, {-3e-1}}, {1, {-2e-1}}, {1, {-1e-1}},
      {1, {0e0}},   {1, {1e-1}},  {1, {2e-1}},  {1, {3e-1}},  {1, {4e-1}},
  ...
  arma::mat YC(NY, NY, arma::fill::zeros);
  ...
  for (auto const& [e, DX] : Campaign)
  {
    ...
    for (size_t k = 0; k < NX; ++k) std::cout << std::setw(12) << DX[k];
    ...
    for (size_t k = 0; k < NY; ++k) exp.output[k].variance = YVAR[k];
    Data.push_back(exp);
  }
  std::cout << std::endl;
