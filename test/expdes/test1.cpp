...
////////////////////////////////////////////////////////////////////////
{
  ...
  for (unsigned int j = 0; j < NY * NS; j++)
    Y[j] = OpODE(j, NC, C.data(), NK, K.data(), &IVP);

  //  mc::FFODE OpODE;
  //  std::vector<mc::FFVar> Y(NY*(NS+1));
  //  for( unsigned int j=0; j<NY*NS; j++ ) Y[j] = OpODE( j, NC, C.data(), NK,
  //  K.data(), &IVP );//, mc::FFODE::SHALLOW );
  // std::cout << DAG;
  ...
  //  DAG.eval( NY*NS, Y.data(), dY.data(), NC, C.data(), dC.data(), NK,
  //  K.data(), dK.data() ); for( unsigned i=0, k=0; i<NS; i++ )
  ...
  */
  ...
  {
    // DAG.eval( NY*NS, Y.data(), dY.data(), NC, C.data(), c.second.data(), NK,
  ...
  */
  ...
  for (auto const& dK : KSAM)
  {
    ...
    DAG.eval(NELE, std::get<3>(D2FMLE), dD2FMLE.data(), NK, K.data(), dK.data(),
             CMLE.size(), CMLE.data(), dCMLE.data(), NYM, YMMLE.data(),
             dYMMLE.data());
    arma::mat dD2FMLEDYDK(NYM, NK);  //, arma::fill::zeros );
    arma::mat dD2FMLEDK2(NK, NK, arma::fill::zeros);
  ...
  for (size_t irep = 0; irep < NREP; ++irep)
  {
    ...
    // double dFMLE;
    // DAG.eval( 1, &FMLE, &dFMLE, CMLE.size(), CMLE.data(), dCMLE.data(), NK,
