...
////////////////////////////////////////////////////////////////////////
{
  ...
  std::vector<std::vector<mc::FFVar>> FCT(
      NS + 1, std::vector<mc::FFVar>(NY * (NS + 1), 0.));  // State functions
  for (size_t i = 0; i < NS; i++)
  {
  ...
  for (unsigned int j = 0; j < NY * NS; j++)
    Y[j] =
        OpODE(j, NC, C.data(), NP, P.data(), &IVP);  //, mc::FFODE::SHALLOW );
  // std::cout << DAG;
  ...
  /*
    ...
    DAG.eval( NY*NS, Y.data(), dY.data(), NC, C.data(), dC.data(), NP, P.data(),
    dP.data() ); for( unsigned i=0, k=0; i<NS; i++ ) for( unsigned j=0; j<NY;
  ...
  */
  ...
  for (auto const& dP : vPSAM)
  {
    ...
    DAG.eval(NELE, std::get<3>(D2FMLE), dD2FMLE.data(), NP, P.data(), dP.data(),
    ...
    for (unsigned k = 0; k < std::get<0>(D2FMLE); ++k)
    {
      ...
      if (i < NP)
        dD2FMLEDP2(i, j) = dD2FMLE[k];
      else
        dD2FMLEDYDP(i - NP, j) = dD2FMLE[k];
    }
    // std::cout << "d2FMLE/dP2 =\n" << dD2FMLEDP2;
  ...
  for (size_t irep = 0; irep < NREP; ++irep)
  {
    ...
    // double dFMLE;
    // DAG.eval( 1, &FMLE, &dFMLE, CMLE.size(), CMLE.data(), dCMLE.data(), NP,
    // P.data(), dP.data() ); std::cout << "FMLE = " << dFMLE << std::endl;
