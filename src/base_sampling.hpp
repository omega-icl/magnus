...
class BASE_SAMPLING
{
 public:
  ...
  //! @brief Set uniform sample within bounds
  template <typename T>
  static void uniform_sample(T& TSAM, size_t NSAM,
  ...
  //! @brief Set Gaussian sample with given mean and variance
  template <typename T>
  static void gaussian_sample(T& TSAM, size_t NSAM,
  ...
  template <typename T>
  static double
  log_sum_exp(std::multimap<double, T> const& points)
  {
    double maxlkh = points.rbegin()->first, sum = 0;
    for (auto const& [lkh, dum] : points) sum += std::exp(lkh - maxlkh);
    return maxlkh + std::log(sum);
  }

  //! @brief Compute the log of the sum of exponentials of two log-likelihood
  //! values
  static double
  log_sum_exp(double const& logZ1, double const& logZ2)
  {
    double maxlogZ = (logZ1 > logZ2 ? logZ1 : logZ2);
    // std::cout << std::scientific << std::setprecision(10);
    // std::cout << "log(Z1) = " << logZ1 << "  log(Z2) = " << logZ2 <<
    // std::endl; std::cout << "log(Z1+Z2) = " << std::log( std::exp( logZ1 ) +
    // std::exp( logZ2 ) )
    //           << " = " << maxlogZ + std::log( std::exp( logZ1 - maxlogZ ) +
    //           std::exp( logZ2 - maxlogZ ) ) << std::endl;
    return maxlogZ +
           std::log(std::exp(logZ1 - maxlogZ) + std::exp(logZ2 - maxlogZ));
  }
...
BASE_SAMPLING::uniform_sample(size_t NSAM, std::vector<double> const& LB,
...
template <typename T>
inline void
BASE_SAMPLING::uniform_sample(T& TSAM, size_t NSAM,
                              std::vector<double> const& LB,
                              std::vector<double> const& UB, bool const sobol)
{
...
inline std::list<std::vector<double>>
BASE_SAMPLING::gaussian_sample(size_t NSAM, std::vector<double> const& M,
                               double const& V, bool const sobol)
{
  std::list<std::vector<double>> LSAM;
...
inline std::list<std::vector<double>>
BASE_SAMPLING::gaussian_sample(size_t NSAM, std::vector<double> const& M,
                               std::vector<double> const& V, bool const sobol)
{
  std::list<std::vector<double>> LSAM;
...
template <typename T>
inline void
BASE_SAMPLING::gaussian_sample(T& TSAM, size_t NSAM,
                               std::vector<double> const& M,
                               std::vector<double> const& V, bool const sobol)
{
  ...
  else
  {
    ...
    if (!mvnrnd(mX, vM, mV, NSAM)) return;
  }
