...
class FFBaseMLE : public FFOp
{
 ...
 public:
  ...
  set(FFGraph* dag, std::vector<FFVar> const& par,
      ...
      std::vector<FFVar> const& out, std::vector<double> const* cstval,
  ...
  // Default constructor
  FFBaseMLE() : FFOp(EXTERN), _DAG(nullptr) {}

  // Destructor
  virtual ~FFBaseMLE() { delete _DAG; }

  // Copy constructor
  FFBaseMLE(FFBaseMLE const& Op) : FFOp(Op), _DAG(nullptr)
  {
    set(Op._DAG, Op._PAR, Op._CST, Op._CON, Op._OUT, Op._DCST, Op._DAT);
  }
};

class FFMLE : public FFBaseMLE
{
 private:
 ...
 public:
      ...
      std::vector<BASE_PAREST::Experiment> const* dat)
  {
    ...
    _DCON.clear();
    _DCON.reserve(_ne);
    for (auto const& EXP : *_DAT) _DCON.push_back(EXP.control);
  }
  ...
  // Copy constructor
  FFMLE(FFMLE const& Op) : FFBaseMLE(Op), _DCON(Op._DCON), _DPAR(Op._DPAR) {}

  // Define operation
  FFVar&
  operator()(FFVar const* var, FFGraph* dag, std::vector<FFVar> const& par,
             std::vector<FFVar> const& cst, std::vector<FFVar> const& con,
             std::vector<FFVar> const& out, std::vector<double> const* cstval,
             std::vector<BASE_PAREST::Experiment> const* dat)
  {
...
class FFGradMLE : public FFBaseMLE
{
 private:
  ...
  void _MLEder(double* gradMLE) const;
 ...
 public:
      ...
      std::vector<BASE_PAREST::Experiment> const* dat)
  {
    ...
    for (auto const& EXP : *_DAT)
    {
      ...
      for (size_t c = 0; c < _nu; ++c)
        std::cout << "_FDCON[" << c << "] =" << (*itFDCON)[c].val()
                  << std::endl;
#endif
      ++itFDCON;
    }
    ...
    _FDCST.clear();
    _FDCST.resize(_nc);
    for (size_t c = 0; c < _nc; ++c) _FDCST[c] = cstval->at(c);
  }
  ...
  // Copy constructor
  FFGradMLE(FFGradMLE const& Op)
      : FFBaseMLE(Op), _FDCON(Op._FDCON), _FDPAR(Op._FDPAR), _FDCST(Op._FDCST)
  {
  }

  // Define operation
  FFVar&
  operator()(size_t const idep, FFVar const* var, FFGraph* dag,
             std::vector<FFVar> const& par, std::vector<FFVar> const& cst,
             std::vector<FFVar> const& con, std::vector<FFVar> const& out,
             std::vector<double> const* cstval,
             std::vector<BASE_PAREST::Experiment> const* dat)
  {
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error(
        "FFGradMLE::feval ** No evaluation method for type" +
        std::string(idU.name()) + "\n");
  }
...
inline void
FFMLE::eval(unsigned const nRes, FFVar* vRes, unsigned const nVar,
            FFVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLE_TRACE
...
inline void
FFMLE::eval(unsigned const nRes, FFDep* vRes, unsigned const nVar,
            FFDep const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLE_TRACE
...
inline void
FFMLE::eval(unsigned const nRes, FFExpr* vRes, unsigned const nVar,
            FFExpr const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLE_TRACE
...
inline void
FFMLE::eval(unsigned const nRes, SLiftVar* vRes, unsigned const nVar,
            SLiftVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLE_TRACE
...
inline void
FFGradMLE::eval(unsigned const nRes, FFVar* vRes, unsigned const nVar,
                FFVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFGRADMLE_TRACE
...
inline void
FFGradMLE::eval(unsigned const nRes, FFDep* vRes, unsigned const nVar,
                FFDep const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFGRADMLE_TRACE
...
inline void
FFGradMLE::eval(unsigned const nRes, FFExpr* vRes, unsigned const nVar,
                FFExpr const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFGRADMLE_TRACE
...
inline void
FFGradMLE::eval(unsigned const nRes, SLiftVar* vRes, unsigned const nVar,
                SLiftVar const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFGRADMLE_TRACE
...
inline void
FFMLE::_MLEval(double& MLE) const
{
#ifdef MC__FFMLE_TRACE
  std::cout << "FFMLE::_MLEval\n";
#endif
#ifdef MC__FFMLE_CHECK
  assert(_DAT && !_DAT->empty() && _DOUT.size() == _ne &&
         _DOUT.front().size() == _ny);
#endif
...
inline void
FFMLE::eval(unsigned const nRes, double* vRes, unsigned const nVar,
            double const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLE_TRACE
  ...
  for (size_t e = 0; e < _ne; ++e)
    if (_nc)
      _DAG->eval(_sgOUT, _wkD, _OUT, _DOUT[e], _CON, _DCON[e], _PAR, _DPAR,
                 _CST, *_DCST);
    else
      _DAG->eval(_sgOUT, _wkD, _OUT, _DOUT[e], _CON, _DCON[e], _PAR, _DPAR);
#endif
...
inline void
FFGradMLE::_MLEder(double* gradMLE) const
{
#ifdef MC__FFMLE_TRACE
  std::cout << "FFGradMLE::_MLEder\n";
#endif
#ifdef MC__FFMLE_CHECK
  assert(_DAT && !_DAT->empty() && _FDOUT.size() == _ne &&
         _FDOUT.front().size() == _ny);
#endif
...
inline void
FFGradMLE::eval(unsigned const nRes, double* vRes, unsigned const nVar,
                double const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFGRADMLE_TRACE
...
#ifdef MC__FFMLE_USE_THREADS
  if (_nc)
    _DAG->veval(_sgOUT, _wkFD, _wkFThd, _OUT, _FDOUT, _CON, _FDCON, _PAR,
                _FDPAR, _CST, _FDCST);
  else
    _DAG->veval(_sgOUT, _wkFD, _wkFThd, _OUT, _FDOUT, _CON, _FDCON, _PAR,
                _FDPAR);
  ...
  for (size_t e = 0; e < _ne; ++e)
    if (_nc)
      _DAG->eval(_sgOUT, _wkFD, _OUT, _FDOUT[e], _CON, _FDCON[e], _PAR, _FDPAR,
                 _CST, _FDCST);
    else
      _DAG->eval(_sgOUT, _wkFD, _OUT, _FDOUT[e], _CON, _FDCON[e], _PAR, _FDPAR);
#endif
...
inline void
FFMLE::eval(unsigned const nRes, fadbad::F<FFVar>* vRes, unsigned const nVar,
            fadbad::F<FFVar> const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLE_TRACE
...
inline void
FFMLE::eval(unsigned const nRes, fadbad::F<double>* vRes, unsigned const nVar,
            fadbad::F<double> const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFMLE_TRACE
...
inline void
FFMLE::deriv(unsigned const nRes, FFVar const* vRes, unsigned const nVar,
             FFVar const* vVar, FFVar** vDer) const
{
#ifdef MC__FFMLE_TRACE
