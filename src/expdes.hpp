...
////////////////////////////////////////////////////////////////////////
class EXPDES : public virtual BASE_MBDOE, protected virtual NSFEAS
{
 protected:
#if defined(MC__USE_PROFIL)
  typedef ::INTERVAL I;
 ...
 public:
  ...
  //! @brief Constructor
  EXPDES() : _dag(nullptr), _dagdoe(nullptr), _VOpt(0. / 0.) { stats.reset(); }
  ...
  struct Options
  {
    ...
    Options&
    operator=(Options const& options)
    {
    ...
    double CVARTHRES;
    //! @brief Uncertainty scenario reduction within threshold (if >=0) or to
    //! k-nearest neighboors (<0)
    double UNCREDUC;
    ...
    size_t FEASPROP;
    //! @brief Minimal relative mean-absolute distance between support points
    //! after refinement
    double MINDIST;
  ...
  struct Stats
  {
    ...
    reset()
    {
      walltime_all = walltime_setup = walltime_samgen = walltime_slvnlp =
          walltime_slvmip = std::chrono::microseconds(0);
    }
    //! @brief Display statistics
    void
    display(std::ostream& os = std::cout)
    {
      os << std::fixed << std::setprecision(2) << std::right << std::endl
         << "#  WALL-CLOCK TIMES" << std::endl
         << "#  SOLVER SETUP:         " << std::setw(10)
         << to_time(walltime_setup) << " SEC" << std::endl
         << "#  SCENARIO GENERATION:  " << std::setw(10)
         << to_time(walltime_samgen) << " SEC" << std::endl
         << "#  EFFORT-BASED SOLVE:   " << std::setw(10)
         << to_time(walltime_slvmip) << " SEC" << std::endl
         << "#  GRADIENT-BASED SOLVE: " << std::setw(10)
         << to_time(walltime_slvnlp) << " SEC" << std::endl
         << "#  TOTAL:                " << std::setw(10)
         << to_time(walltime_all) << " SEC" << std::endl
         << std::endl;
    }
    //! @brief Total wall-clock time (in microseconds)
    std::chrono::microseconds walltime_all;
    //! @brief Cumulated wall-clock time used for problem setup (in
    //! microseconds)
    std::chrono::microseconds walltime_setup;
    //! @brief Cumulated wall-clock time used by sample generation and scenario
    //! reduction (in microseconds)
    std::chrono::microseconds walltime_samgen;
    //! @brief Cumulated wall-clock time used by gradient-based NLP solver (in
    //! microseconds)
    std::chrono::microseconds walltime_slvnlp;
    //! @brief Cumulated wall-clock time used by effort-based MINLP solver (in
    //! microseconds)
    ...
    lapse(std::chrono::time_point<std::chrono::system_clock> const& start) const
    {
    ...
    double
    to_time(std::chrono::microseconds t) const
    {
      return t.count() * 1e-6;
    }
  } stats;
  ...
  bool setup(size_t const ndxmod = 0);
  ...
  std::tuple<double, std::vector<double>, bool> evaluate_design(
      ...
      std::vector<double> const& vcst = std::vector<double>(),
      std::ostream& os                = std::cout);

  //! @brief Generate <a>NSAM</a> initial supports
  bool sample_support(size_t const NSAM,
                      std::vector<double> const& vcst = std::vector<double>(),
                      std::ostream& os                = std::cout);

  //! @brief Solve effort-based exact experiment design with <a>NEXP</a>
  //! supports
  int effort_solve(
  ...
  //! @brief Solve effort-based exact experiment design with increasing number
  //! of supports <a>vNEXP</a>
  int effort_solve(
      std::vector<size_t> const& vNEXP, bool const exact = true,
      std::map<size_t, double> const& EIni = std::map<size_t, double>(),
      std::ostream& os                     = std::cout);

  //! @brief Solve gradient-based experiment design for refinement of
  //! <a>EOpt</a> supports
  int gradient_solve(std::map<size_t, double> const& EOpt,
                     std::vector<double> const& vcst = std::vector<double>(),
  ...
  //! @brief Solve combined effort- and gradient-based experiment design with
  //! <a>NEXP</a> supports
  int combined_solve(
      size_t const NEXP,
      std::vector<double> const& vcst      = std::vector<double>(),
  ...
  bool file_export(std::string const& name);
  ...
  std::map<size_t, std::vector<double>> const&
  support() const
  {
    return _SOpt;
  }
  ...
  std::list<std::pair<double, std::vector<double>>> campaign() const;

  //! @brief Retrieve sampled controls
  std::vector<std::vector<double>> const&
  control_sample() const
  {
    return _vCONSAM;
  }

  //! @brief Retrieve sampled outputs
  std::vector<std::vector<arma::vec>> const&
  output_sample() const
  {
    return _vOUTSAM;
  }
  ...
  std::vector<std::vector<arma::mat>> const&
  fim_sample() const
  {
    return _vFIMSAM;
  }
 ...
 protected:
  ...
  void _setup_out(size_t const ndxmod);

  //! @brief Create local copy of output model for FIM prediction
  void _setup_fim(size_t const ndxmod);

  //! @brief Generate <a>NSAM</a> feasible supports
  bool _sample_support_nsfeas(size_t const NSAM, std::ostream& os);

  //! @brief Generate output samples for <a>NSAM</a> initial supports
  bool _sample_out(size_t const NSAM, std::ostream& os = std::cout);

  //! @brief Select uncertainty scenario subset based on highest value
  void _sample_rank(std::set<std::pair<size_t, size_t>>& parsel,
                    std::vector<double> const& E0, std::ostream& os);

  //! @brief Select uncertainty scenario subset based on nearest-neighbors
  //! criterion
  void _sample_select(size_t const start, size_t const inc,
                      std::set<std::pair<size_t, size_t>>& parsel,
                      std::vector<double> const& E0, std::ostream& os);

  //! @brief Append output under given control scenario for all uncertainty
  //! scenarios
  bool _append_out(std::vector<double> const& Control,
                   std::vector<std::vector<double>> const& Parameter,
                   std::vector<std::vector<double>>& Output,
                   std::vector<std::vector<arma::vec>>& Response,
                   std::ostream& os = std::cout);

  //! @brief Generate FIM samples for <a>NSAM</a> initial supports
  bool _sample_fim(size_t const NSAM, std::ostream& os = std::cout);

  //! @brief Append FIM (columnwise, lower-triangular) under given control
  //! scenario for all uncertainty scenarios
  bool _append_fim(std::vector<double> const& Control,
                   std::vector<std::vector<double>> const& Parameter,
                   std::vector<std::vector<double>>& FIM,
                   std::vector<std::vector<arma::mat>>& Response,
                   std::ostream& os = std::cout);

  //! @brief Set Bayes Risk criterion in effort-based MINLP model
  void _effort_set_BRisk(MINLP& doe, FFVar const& NEXP,
                         std::vector<FFVar> const& EFF) const;

  //! @brief Set output distance criterion in effort-based MINLP model
  void _effort_set_ODist(MINLP& doe, FFVar const& NEXP,
                         std::vector<FFVar> const& EFF, std::vector<double>& E0,
                         std::vector<I>& EBND) const;

  //! @brief Set FIM-based risk-neutral criterion in effort-based MINLP model
  void _effort_set_FIMNeutral(MINLP& doe, FFVar const& NEXP,
                              std::vector<FFVar> const& EFF) const;

  //! @brief Set FIM-based risk-averse criterion in effort-based MINLP model
  void _effort_set_FIMAverse(MINLP& doe, FFVar const& NEXP,
                             std::vector<FFVar> const& EFF,
                             std::vector<double>& E0,
                             std::vector<I>& EBND) const;

  //! @brief Evaluate constraints
  void _evaluate_constraints(std::vector<double>& DCTR, size_t const NSUP,
                             size_t const NUNC, double const* DCTRSAM,
                             std::ostream& os);
  ...
  void _refine_set_constraints(NLP& doeref, size_t const NE,
                               FFVar const* const* ppCTRSAM,
                               std::vector<double>& UTOT0, std::ostream& os);

  //! @brief Set Bayes Risk criterion in gradient-based NLP model
  void _refine_set_BRisk(NLP& doeref, std::map<size_t, double> const& EOpt,
                         std::vector<FFVar> const& UTOT,
                         std::vector<double>& UTOT0, std::ostream& os);

  //! @brief Set output distance criterion in gradient-based NLP model
  void _refine_set_ODist(NLP& doeref, std::map<size_t, double> const& EOpt,
                         std::vector<FFVar> const& UTOT,
                         std::vector<double>& UTOT0, std::ostream& os);

  //! @brief Set FIM-based risk-neutral criterion in gradient-based NLP model
  void _refine_set_FIMNeutral(NLP& doeref, std::map<size_t, double> const& EOpt,
                              std::vector<FFVar> const& UTOT,
                              std::vector<double>& UTOT0, std::ostream& os);

  //! @brief Set FIM-based risk-averse criterion in gradient-based NLP model
  void _refine_set_FIMAverse(NLP& doeref, std::map<size_t, double> const& EOpt,
                             std::vector<FFVar> const& UTOT,
                             std::vector<double>& UTOT0, std::ostream& os);

  //! @brief Run gradient-based refinement
  int _gradient_solve(std::map<size_t, double> const& EOpt,
                      std::vector<double>& UTOT0,
                      std::vector<double> const& UTOTLB,
                      std::vector<double> const& UTOTUB,
                      std::vector<double> const& vcst, bool const update,
                      std::ostream& os);

  //! @brief Generate samples for refined supports
  bool _update_supports(std::map<size_t, double> const EOpt,
                        std::map<size_t, std::vector<double>> const SOpt,
                        std::ostream& os);

  //! @brief Determine if support <a>supp</a> is redundant with an existing
  //! support
  size_t _redundant_support(std::vector<double> const& supp);

  //! @brief Mean-absolute error between two supports
  double _mae_support(std::vector<double> const& s1,
                      std::vector<double> const& s2);

  //! @brief Display current efforts and supports
  void _display_design(std::string const& title, double const& crit,
                       std::map<size_t, double> const& eff,
                       std::map<size_t, std::vector<double>> const& supp,
...
inline bool
EXPDES::setup(size_t const ndxmod)
{
  stats.reset();
  auto&& t_setup = stats.start();

  if (!_ny) throw Exceptions(Exceptions::NOMODEL);
...
inline void
EXPDES::_setup_out(size_t const ndxmod)
{
  if (ndxmod >= _nm || !_ny || _ny != BASE_MBDOE::_vOUT[ndxmod].size() ||
      !BASE_MBDOE::_vCON.size())
  ...
  _vOUT.resize(_ny);
  _dag->insert(BASE_MBDOE::_dag, _ny, BASE_MBDOE::_vOUT[ndxmod].data(),
               _vOUT.data());
  _sgOUT.clear();
...
inline void
EXPDES::_setup_fim(size_t const ndxmod)
{
  if (ndxmod >= _nm || !_ny || _ny != BASE_MBDOE::_vOUT[ndxmod].size() ||
      !BASE_MBDOE::_vCON.size())
    throw Exceptions(Exceptions::BADSIZE);
  ...
  FFVar* y_p             = _dag->FAD(_ny, _vOUT.data(), _np, _vPAR.data(),
                                     true);  // Jacobian in dense format
  ...
  for (size_t i = 0, ij = 0; i < _np; ++i)
    for (size_t j = i; j < _np; ++j, ++ij) _vFIM[ij] = *ppFIM[ij];
#else
...
inline bool
EXPDES::_sample_support_nsfeas(size_t const NSAM, std::ostream& os)
{
  ...
  if (options.DISPLEVEL) NSFEAS::stats.display();
  ...
  {
    _vCONSAM.push_back(
        std::vector<double>(std::get<0>(pcon), std::get<0>(pcon) + _nu));
...
inline bool
EXPDES::sample_support(size_t const NSAM, std::vector<double> const& vcst,
                       std::ostream& os)
{
  ...
  if (options.DISPLEVEL) os << std::endl;
...
inline bool
EXPDES::_sample_out(size_t const NSAM, std::ostream& os)
{
  ...
  {
    if (!_append_out(_vCONAP[s], _vPARVAL, _dOUT, _vOUTSAM, os)) return false;
    if (options.DISPLEVEL && !(s % DISPFREQ)) os << "=" << std::flush;
  }

  // Compute responses for every control samples and uncertainty scenarios
  for (size_t s = 0; s < NSAM; ++s)
  {
    if (!_append_out(_vCONSAM[s], _vPARVAL, _dOUT, _vOUTSAM, os)) return false;
    if (options.DISPLEVEL && !((s + _ne0) % DISPFREQ)) os << "=" << std::flush;
  }
  if (options.DISPLEVEL)
    os << "| " << NUNC * (_ne0 + NSAM) << std::right << std::fixed
       << std::setprecision(2) << std::setw(10)
       << stats.to_time(stats.lapse(tstart)) << " SEC" << std::flush;

  if (!NSAM || options.CRITERION == ODIST || options.UNCREDUC == 0 ||
      options.UNCREDUC >= 1. || -options.UNCREDUC >= NUNC - 1)
    return true;

  // Perform scenario reduction for Bayes risk
  tstart = stats.start();
  if (options.DISPLEVEL)
    os << std::endl << "** REDUCING UNCERTAINTY SCENARIOS | " << std::flush;
  ...
  if (options.UNCREDUC > 0.) _sample_rank(_sPARSEL, E0, os);
  ...
  else
  {
    ...
    size_t const NOTHREADS =
        (_dag->options.MAXTHREAD > 0 ? _dag->options.MAXTHREAD
                                     : std::thread::hardware_concurrency());
    std::vector<std::thread> vth(NOTHREADS -
                                 1);  // Main thread also runs evaluations
    ...
    for (size_t th = 1; th < NOTHREADS; th++)
      vth[th - 1] =
          std::thread(&EXPDES::_sample_select, this, th, NOTHREADS,
                      std::ref(vsel[th - 1]), std::cref(E0), std::ref(os));
  ...
  if (options.DISPLEVEL)
    os << _sPARSEL.size() << " PAIRS" << std::right << std::fixed
       << std::setprecision(2) << std::setw(10)
       << stats.to_time(stats.lapse(tstart)) << " SEC" << std::flush;

#ifdef MAGNUS__EXPDES_SAMPLE_DEBUG
  std::cout << "PARSEL[" << _sPARSEL.size() << "]: ";
  for (auto const& [j, k] : _sPARSEL) std::cout << " (" << j << "," << k << ")";
  std::cout << std::endl;
#endif
...
inline void
EXPDES::_sample_select(size_t const start, size_t const inc,
                       std::set<std::pair<size_t, size_t>>& parsel,
                       std::vector<double> const& E0, std::ostream& os)
{
  ...
  int DISPFREQ =
      (_vOUTSAM.size() - start > 20 ? (_vOUTSAM.size() - start) / 20 : 1);

  for (size_t j = start; j < _vOUTSAM.size(); j += inc)
  {
    if (options.DISPLEVEL && !(j % DISPFREQ)) os << "=" << std::flush;

    for (size_t k = 0, kk = 0; k < _vOUTSAM.size(); ++k)
      if (k != j)
        BRall[kk++] = std::make_pair(
            k, FFDOEBase::atom_BR(_vOUTSAM.at(j), _vOUTSAM.at(k), E0));
    auto BRgt = [](std::pair<size_t, double> const& a,
                   std::pair<size_t, double> const& b)
    { return a.second > b.second; };
    assert(std::partial_sort_copy(BRall.begin(), BRall.end(), BRtop.begin(),
                                  BRtop.end(), BRgt) == BRtop.end());
  ...
  if (options.DISPLEVEL) os << "|" << std::flush;
}

inline void
EXPDES::_sample_rank(std::set<std::pair<size_t, size_t>>& parsel,
                     std::vector<double> const& E0, std::ostream& os)
{
  // Select the pair that contribute (1-options.UNCREDUC)*100% of the total
  // Bayes Risk
  size_t NUNC = _vPARVAL.size();
  std::vector<std::pair<std::pair<size_t, size_t>, double>> BRall(
      NUNC * (NUNC + 1) / 2);
  ...
  for (size_t j = 0, kk = 0; j < NUNC; ++j)
  {
    for (size_t k = j + 1; k < NUNC; ++k, ++kk)
    {
      BRall[kk] = std::make_pair(
          std::make_pair(j, k),
          FFDOEBase::atom_BR(_vOUTSAM.at(j), _vOUTSAM.at(k), E0));
      BRtot += BRall[kk].second;
    }
  }

  auto BRgt = [](std::pair<std::pair<size_t, size_t>, double> const& a,
                 std::pair<std::pair<size_t, size_t>, double> const& b)
  { return a.second > b.second; };
  ...
  for (auto const& [jk, val] : BRall)
  {
    BRsum += val;
    parsel.insert(jk);
    // std::cerr << "(" << jk.first << "," << jk.second << "," << val << ") " <<
    // BRsum/BRtot*1e2 << "%\n";
...
inline bool
EXPDES::_append_out(std::vector<double> const& Control,
                    std::vector<std::vector<double>> const& Parameter,
                    std::vector<std::vector<double>>& Output,
                    std::vector<std::vector<arma::vec>>& Response,
                    std::ostream& os)
{
  ...
  {
    if (_nc)
      _dag->veval(_sgOUT, _wkOUT, _vOUT, Output, _vPAR, Parameter, _vCON,
                  Control, _vCST, _vCSTVAL);
    else
      _dag->veval(_sgOUT, _wkOUT, _vOUT, Output, _vPAR, Parameter, _vCON,
                  Control);
  }
  ...
  for (size_t k = 0; k < Output.size(); ++k)
  {
    ...
    if (Response[k].size() &&
        arma::size(Response[k].back()) != arma::size(vOut))
      throw Exceptions(Exceptions::BADSIZE);
    Response[k].push_back(vOut);
#ifdef MAGNUS__EXPDES_SAMPLE_DEBUG
    std::cout << "OUT[" << k << "][" << Response[k].size() << "]:" << std::endl
              << arma::trans(vOut);
#endif
  }
...
inline bool
EXPDES::_sample_fim(size_t const NSAM, std::ostream& os)
{
  auto&& tstart = stats.start();
  int DISPFREQ  = (_ne0 + NSAM > 20 ? (_ne0 + NSAM) / 20 : 1);
  if (options.DISPLEVEL)
  ...
  for (size_t s = 0; s < _ne0; ++s)
  {
    if (!_append_fim(_vCONAP[s], _vPARVAL, _dFIM, _vFIMSAM, os)) return false;
    if (options.DISPLEVEL && !(s % DISPFREQ)) os << "=" << std::flush;
  }
  ...
  for (size_t s = 0; s < NSAM; ++s)
  {
    if (!_append_fim(_vCONSAM[s], _vPARVAL, _dFIM, _vFIMSAM, os)) return false;
    if (options.DISPLEVEL && !((s + _ne0) % DISPFREQ)) os << "=" << std::flush;
  }
  if (options.DISPLEVEL)
    os << "| " << NUNC * (_ne0 + NSAM) << std::right << std::fixed
  ...
  if (options.FIMSTOL <= 0e0) return true;

  tstart = stats.start();
  if (options.DISPLEVEL)
    os << std::endl << "** CHECKING FIM REGULARITY        | " << std::flush;
...
#ifdef MAGNUS__EXPDES_SAMPLE_DEBUG
    std::cout << "FIM[" << s << "]: rank = " << arma::rank(FIMs) << std::endl
              << FIMs;
#endif
    if (_vPARWEI.size() == NUNC)
      FIMs *= _vPARWEI[s];  // uncertainty scenario probability
    FIM += FIMs;
  }
  if (_mPARSCA.n_elem)
    FIM = arma::trans(_mPARSCA) * FIM * _mPARSCA;  // parameter scaling factors
#ifdef MAGNUS__EXPDES_SAMPLE_DEBUG
  ...
  if (options.DISPLEVEL)
    os << PROJ.n_rows - PROJ.n_cols << " SINGULAR VALUES BELOW THRESHOLD ("
       << std::scientific << options.FIMSTOL << ")" << std::right << std::fixed
       << std::setprecision(2) << std::setw(10)
       << stats.to_time(stats.lapse(tstart)) << " SEC" << std::flush;

  // USE ARMADILLO FUNCTIONS NULL AND ORTH:
  // https://arma.sourceforge.net/docs.html#orth os << "  FIM is regular on the
  // experimental space discretization" << std::endl; os << "  FIM is
  // rank-defficient on the experimental space discretization - Rank = " <<
  // std::endl; os << "  x relationships between parameters... " << std::endl;

  return true;
}

inline bool
EXPDES::_append_fim(std::vector<double> const& Control,
                    std::vector<std::vector<double>> const& Parameter,
                    std::vector<std::vector<double>>& FIM,
                    std::vector<std::vector<arma::mat>>& Response,
                    std::ostream& os)
    ...
    for (size_t i = 0, l = 0; i < _np; ++i)
      for (size_t j = i; j < _np; ++j, ++l)
        if (i == j)
          ...
          mFIM(i, j) = mFIM(j, i) = FIM[k][l];
    if (Response[k].size() &&
        arma::size(Response[k].back()) != arma::size(mFIM))
      throw Exceptions(Exceptions::BADSIZE);
    Response[k].push_back(mFIM);
#ifdef MAGNUS__EXPDES_SAMPLE_DEBUG
    std::cout << "FIM[" << k << "][" << Response[k].size() << "]:" << std::endl
              << mFIM;
#endif
  }
...
inline size_t
EXPDES::_redundant_support(std::vector<double> const& suppref)
{
  size_t pos = 0;
  for (auto const& supp : _vCONSAM)
  {
    if (_mae_support(supp, suppref) < options.MINDIST) return pos;
    ++pos;
  }
  return pos;
}
...
inline double
EXPDES::_mae_support(std::vector<double> const& s1,
                     std::vector<double> const& s2)
{
  if (s1.size() != s2.size() || s1.size() != _vCONLB.size())
    return 0. / 0.;  // NaN
  double mae = 0;
  for (auto it1 = s1.cbegin(), it2 = s2.cbegin(), itLB = _vCONLB.cbegin(),
            itUB = _vCONUB.cbegin();
       it1 != s1.end(); ++it1, ++it2, ++itLB, ++itUB)
    mae += std::fabs(*it1 - *it2) / std::fabs(*itUB - *itLB);
  mae /= s1.size();
#ifdef MAGNUS__EXPDES_SAMPLE_DEBUG
  std::cout << "mae: " << std::scientific << std::setprecision(7) << mae
            << std::endl;
#endif
  return mae;
}
...
inline bool
EXPDES::_update_supports(std::map<size_t, double> const EOpt,
                         std::map<size_t, std::vector<double>> const SOpt,
                         std::ostream& os)
{
  auto&& t_samgen = stats.start();
  if (options.DISPLEVEL) os << "** REFINING SUPPORT SAMPLES" << std::endl;
  ...
  for (size_t s = posSupp; s < posSupp + newSupp; ++s)
  {
    ...
    if (options.DISPLEVEL > 1) os << "." << std::flush;
  }
  if (options.DISPLEVEL > 1) os << std::endl;
...
inline bool
EXPDES::file_export(std::string const& name)
{
...
inline int
EXPDES::combined_solve(size_t const NEXP, std::vector<double> const& vcst,
                       bool const exact, std::map<size_t, double> const& EIni,
                       std::ostream& os)
{
  _EOpt        = EIni;
  double VLast = 0.;
  ...
  for (int it = 0;;)
  {
    ...
    if (it && std::fabs(VLast - _VOpt) <
                  options.TOLITER * std::fabs(VLast + _VOpt) / 2)
    {
      if (options.DISPLEVEL)
...
inline void
EXPDES::_effort_set_BRisk(MINLP& doe, FFVar const& NEXP,
                          std::vector<FFVar> const& EFF) const
{
  FFLin<I> Sum;
...
inline void
EXPDES::_effort_set_ODist(MINLP& doe, FFVar const& NEXP,
                          std::vector<FFVar> const& EFF,
                          std::vector<double>& E0, std::vector<I>& EBND) const
{
  FFLin<I> Sum;
  doe.add_ctr(BASE_OPT::EQ, Sum(EFF, 1.) - NEXP);
  ...
  for (auto& Sk : SEL) Sk.set(_dagdoe);
  ...
  for (size_t i = 0; i < NSUPP; i++)
  {
    ...
    // FFVar vODISTEff = OpODISTEff( SELi, NEXP, i, options.IDWTOL, &_vOUTSAM,
    // &_vEFFAP );
    FFVar vODISTEff =
        OpODISTEff(SEL, NEXP, i, options.IDWTOL, &_vOUTSAM, &_vEFFAP);
...
inline void
EXPDES::_effort_set_FIMNeutral(MINLP& doe, FFVar const& NEXP,
                               std::vector<FFVar> const& EFF) const
{
  FFLin<I> Sum;
  doe.add_ctr(BASE_OPT::EQ, Sum(EFF, 1.) - NEXP);
  ...
  doe.set_obj(BASE_OPT::MAX,
              Sum(NUNC, OpFIMEff(EFF, &_vFIMSAM, &_vEFFAP), _vPARWEI.data()));
}

inline void
EXPDES::_effort_set_FIMAverse(MINLP& doe, FFVar const& NEXP,
                              std::vector<FFVar> const& EFF,
                              std::vector<double>& E0,
                              std::vector<I>& EBND) const
{
  ...
  E0.insert(E0.end(), 1, 0e0);
  if (!EBND.empty())
    EBND.insert(EBND.end(), 1, I(-BASE_OPT::INF, BASE_OPT::INF));

  std::vector<FFVar> DELTA(NUNC);
  for (auto& Dk : DELTA) Dk.set(_dagdoe);
...
inline int
EXPDES::effort_solve(std::vector<size_t> const& vNEXP, bool const exact,
                     std::map<size_t, double> const& EIni, std::ostream& os)
{
  ...
  for (auto& Ek : EFF) Ek.set(_dagdoe);
  std::vector<double> E0;
  if (EIni.empty())
  ...
  {
    E0.assign(NSUPP, 0e0);
    for (auto const& [ndx, eff] : EIni) E0[ndx] = eff;
  }

  // Total experiment count and default bounds
  size_t TOTEXP =
      std::accumulate(std::next(vNEXP.cbegin()), vNEXP.cend(), vNEXP.front());
  ...
  for (auto const& NEXP : vNEXP)
  {
    ...
    if (doe.get_status() != MINLP::SUCCESSFUL) break;
  }

  if (options.DISPLEVEL > 1) doe.stats.display();
  ...
  if (options.DISPLEVEL)
    if (exact)
      ...
      _display_design("EFFORT-BASED CONTINUOUS DESIGN", _VOpt, _EOpt, _SOpt,
                      os);
...
inline int
EXPDES::effort_solve(size_t const NEXP, bool const exact,
                     std::map<size_t, double> const& EIni, std::ostream& os)
{
  auto&& t_slvmip = stats.start();
...
inline std::list<std::pair<double, std::vector<double>>>
EXPDES::campaign() const
{
...
inline void
EXPDES::_evaluate_constraints(std::vector<double>& DCTR, size_t const NSUP,
                              size_t const NUNC, double const* DCTRSAM,
                              std::ostream& os)
{
  if (!_ng) return;
  ...
  for (size_t e = 0; e < NSUP; ++e)
  {
    ...
    if (NUNC == 1)
    {
      ...
      else
      {
        ...
        std::cout << "DCTRSAM[" << e * _ng << ":" << (e + 1) * _ng - 1
                  << "] = " << arma::rowvec(DCTRSAM + e * _ng, _ng, false);
#endif
        auto itMax =
            std::max_element(DCTRSAM + e * _ng, DCTRSAM + (e + 1) * _ng);
        DCTR[e] = *itMax;
      }
#ifdef MAGNUS__EXPDES_EVAL_DEBUG
    ...
    for (size_t s = 0; s < NUNC; ++s)
    {
      auto itMax = std::max_element(DCTRSAM + e * _ng * NUNC + s * _ng,
                                    DCTRSAM + e * _ng * NUNC + (s + 1) * _ng);
      ResCTR.insert(
          {-*itMax,
           _vPARWEI[s]});  // ordered by largest (negative) violation first
    }
...
EXPDES::_evaluate_ODist(std::map<size_t, double> const& EOpt,
                        std::vector<FFVar> const& UTOT,
                        std::vector<double> const& UTOT0, std::ostream& os)
{
  ...
  FFVar const* const* ppODCrit =
      OpODCrit(UTOT.data(), _dag, &_vPAR, &_vCST, &_vCON, &_vOUT, &_vCTR, &EOpt,
  ...
  for (size_t i = 0; i < UTOT.size(); i++)
  {
    ...
    std::cout << "d_UTOT[" << i << "] = " << (UTOT0_pert[i] - UTOT0[i])
              << std::endl;
    std::cout << "gradFD OD[" << i << "] =";
    for (size_t s = 0; s < NSUP * (1 + _ng * NUNC); ++s)
      std::cout << "  "
                << (DODCrit_pert[s] - DODCrit[s]) / (UTOT0_pert[i] - UTOT0[i]);
    std::cout << std::endl;
  }
...
inline std::pair<double, std::vector<double>>
EXPDES::_evaluate_BRisk(std::map<size_t, double> const& EOpt,
                        std::vector<FFVar> const& UTOT,
                        std::vector<double> const& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  ...
  FFBRCrit OpBRCrit;
  FFVar const* const* ppBRCrit =
      OpBRCrit(UTOT.data(), _dag, &_vPAR, &_vCST, &_vCON, &_vOUT, &_vCTR, &EOpt,
  ...
  for (size_t s = 0; s < 1 + NSUP * _ng * NUNC; ++s) FBRCrit[s] = *ppBRCrit[s];
  ...
  for (size_t i = 0; i < UTOT.size(); i++)
  {
    ...
    for (size_t s = 0; s < 1 + NSUP * _ng * NUNC; ++s)
      std::cout << "  "
                << (DBRCrit_pert[s] - DBRCrit[s]) / (UTOT0_pert[i] - UTOT0[i]);
    std::cout << std::endl;
  }
...
inline std::pair<double, std::vector<double>>
EXPDES::_evaluate_FIMNeutral(std::map<size_t, double> const& EOpt,
                             std::vector<FFVar> const& UTOT,
                             std::vector<double> const& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  ...
  FFFIMCrit OpFIMCrit;
  FFVar const* const* ppFIMCrit =
      OpFIMCrit(UTOT.data(), _dag, &_vPAR, &_vCST, &_vCON, &_vFIM, &_vCTR,
                &EOpt, &_vPARVAL, &_vCSTVAL, &_vFIMSAM, &_vEFFAP);
#ifdef MAGNUS__EXPDES_SOLVE_DEBUG
  ...
  for (size_t i = 0; i < UTOT.size(); i++)
  {
    ...
    for (size_t s = 0; s < 1 + NSUP * _ng * NUNC; ++s)
      std::cout << "  "
                << (DFIMCrit_pert[s] - DFIMCrit[s]) /
                       (UTOT0_pert[i] - UTOT0[i]);
    std::cout << std::endl;
  }
...
inline std::pair<double, std::vector<double>>
EXPDES::_evaluate_FIMAverse(std::map<size_t, double> const& EOpt,
                            std::vector<FFVar> const& UTOT,
                            std::vector<double> const& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  ...
  for (size_t s = 0; s < NUNC; ++s) SA.insert({DFIMCrit[s], _vPARWEI[s]});
...
inline std::tuple<double, std::vector<double>, bool>
EXPDES::evaluate_design(
    std::list<std::pair<double, std::vector<double>>> const& Campaign,
    std::string const& type, std::vector<double> const& vcst, std::ostream& os)
{
  ...
  for (size_t i = 0; i < NUTOT; i++) UTOT[i].set(_dagdoe);
  ...
  // Evaluate design
  std::string header(type.empty() ? "DESIGN PERFORMANCE"
  ...
  {
    if (options.DISPLEVEL)
      _display_design(header, crit.first,
                      std::list<std::pair<double, std::vector<double>>>(), os);
    return std::make_tuple(crit.first, crit.second, false);  // NaN
  }

  if (options.DISPLEVEL) _display_design(header, crit.first, Campaign, os);
  return std::make_tuple(crit.first, crit.second, true);
}

inline void
EXPDES::_refine_set_constraints(NLP& doeref, size_t const NE,
                                FFVar const* const* ppCTRSAM,
                                std::vector<double>& UTOT0, std::ostream& os)
{
  if (!_ng) return;
  ...
  for (size_t e = 0; e < NE; ++e)
  {
    // Same VaR and delta vector shared between constraints since CVaR is on the
    // max violation
    ...
    for (size_t g = 0; g < NG; ++g)
    {
      doeref.add_ctr(BASE_OPT::LE,
                     VaRg + Sum(NS, Dg.data(), _vPARWEI.data()) /
                                options.FEASTHRES);  // enforce CVaR constraint
      for (size_t s = 0; s < NS; s++)
        doeref.add_ctr(BASE_OPT::GE,
                       VaRg + Dg[s] - *ppCTRSAM[(e * NS + s) * NG + g]);
    }
  }
}

inline void
EXPDES::_refine_set_ODist(NLP& doeref, std::map<size_t, double> const& EOpt,
                          std::vector<FFVar> const& UTOT,
                          std::vector<double>& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  if (!_vEFFAP.empty() && _vOUTSAM.empty()) _sample_out(0, os);
  ...
  if (_ROpt.size() == NC + 1)
    UTOT0.push_back(
        _ROpt.back());  // last element recorded from effort_solve is OD
  else
    UTOT0.push_back(0e0);
...
inline void
EXPDES::_refine_set_BRisk(NLP& doeref, std::map<size_t, double> const& EOpt,
                          std::vector<FFVar> const& UTOT,
                          std::vector<double>& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
...
inline void
EXPDES::_refine_set_FIMNeutral(NLP& doeref,
                               std::map<size_t, double> const& EOpt,
                               std::vector<FFVar> const& UTOT,
                               std::vector<double>& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  if (!_vEFFAP.empty() && _vFIMSAM.empty()) _sample_fim(0, os);
...
inline void
EXPDES::_refine_set_FIMAverse(NLP& doeref, std::map<size_t, double> const& EOpt,
                              std::vector<FFVar> const& UTOT,
                              std::vector<double>& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  ...
  else
  {
    ...
    doeref.set_obj(
        BASE_OPT::MAX,
        VaR -
            Sum(NS, DELTA.data(), _vPARWEI.data()) /
                options.CVARTHRES);  // maximize risk-averse FIM-based criterion
    for (size_t s = 0; s < NS; s++)
...
inline int
EXPDES::gradient_solve(std::map<size_t, double> const& EOpt,
                       std::vector<double> const& vcst, bool const update,
                       std::ostream& os)
{
  // Initial guess and control bounds
...
#ifdef MAGNUS__EXPDES_SOLVE_DEBUG
    std::cout << "U[" << ndx << "] = ";
    for (size_t i = 0; i < _nu; i++) std::cout << _vCONSAM[ndx][i] << "  ";
    std::cout << std::endl;
#endif
    UTOTLB.insert(UTOTLB.end(), _vCONLB.cbegin(), _vCONLB.cend());
    UTOTUB.insert(UTOTUB.end(), _vCONUB.cbegin(), _vCONUB.cend());
...
inline int
EXPDES::_gradient_solve(std::map<size_t, double> const& EOpt,
                        std::vector<double>& UTOT0,
                        std::vector<double> const& UTOTLB,
                        std::vector<double> const& UTOTUB,
                        std::vector<double> const& vcst, bool const update,
                        std::ostream& os)
{
  ...
  size_t const NSUP  = EOpt.size();
  size_t const NUTOT = _nu * NSUP;
  std::vector<FFVar> UTOT =
      _dagdoe->add_vars(NUTOT, "C");  // Concatenated experimental controls
      ...
      doeref.get_status() == NLP::INTERRUPTED)
  {
    ...
    if (NEXTRA > 0) _ROpt.assign(dC, dC + NEXTRA);
  }
  else
  {
    _EOpt.clear();
...
inline void
EXPDES::_display_design(std::string const& title, double const& crit,
                        std::map<size_t, double> const& eff,
                        std::map<size_t, std::vector<double>> const& supp,
                        std::ostream& os) const
{
  ...
  {
    os << "   SUPPORT #" << i << ": " << std::fixed << std::setprecision(2)
       << eff.at(i) << " x [ " << std::scientific << std::setprecision(5);
    for (auto Ck : s) os << Ck << " ";
    os << "]" << std::endl;
  }
  os << std::endl;
    ...
    std::ostream& os) const
{
  ...
  {
    os << "   SUPPORT #" << i++ << ": " << std::fixed << std::setprecision(2)
       << eff << " x [ " << std::scientific << std::setprecision(5);
    for (auto Ck : supp) os << Ck << " ";
    os << "]" << std::endl;
  }
  os << std::endl;
}
