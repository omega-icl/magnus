...
class FFNSamp : public FFOp
{
 private:
  ...
  void _feasval(double* CRIT, std::vector<std::vector<double>>& DFCT) const;
  // Evaluation of likelihood criterion
  void _lkhdval(double* CRIT, std::vector<std::vector<double>>& DFCT) const;
 ...
 public:
  ...
  void
  set(FFGraph* dag, std::vector<FFVar> const* con,
      std::vector<FFVar> const* par, std::vector<FFVar> const* cst,
      std::vector<FFVar> const* ctr, FFVar const* lkh,
      std::vector<std::vector<double>> const* vpar,
      std::vector<double> const* wpar, std::vector<double> const* vcst)
  {
  ...
  // Default constructor
  FFNSamp(double const& ConfCTR_ = 0.10, double const& ConfLKH_ = 0.10)
      : FFOp(EXTERN), _DAG(nullptr)
  {
    ConfCTR = ConfCTR_;
    ConfLKH = ConfLKH_;
  }
  ...
  // VaR and CVaR are in reference to confidence threshold in static public
  // members mc::FFNSamp::ConfCTR and mc::FFNSamp::confLKH For a single
  // parameter scenario or no parameter dependence, VaR and CVaR are equal to
  // the constraint/likelihood value
  FFVar**
  operator()(FFGraph* dag, std::vector<FFVar> const* con,
             std::vector<FFVar> const* par, std::vector<FFVar> const* cst,
             std::vector<FFVar> const* ctr, FFVar const* lkh,
             std::vector<std::vector<double>> const* vpar,
             std::vector<double> const* wpar, std::vector<double> const* vcst)
  ...
  FFVar&
  operator()(unsigned const idep, FFGraph* dag, std::vector<FFVar> const* con,
             std::vector<FFVar> const* par, std::vector<FFVar> const* cst,
             std::vector<FFVar> const* ctr, FFVar const* lkh,
             std::vector<std::vector<double>> const* vpar,
             std::vector<double> const* wpar, std::vector<double> const* vcst)
        ...
        unsigned const nVar, void const* vVar, unsigned const* mVar) const
  {
    ...
    throw std::runtime_error("FFNSamp::feval ** No evaluation method for type" +
                             std::string(idU.name()) + "\n");
  }
...
inline void
FFNSamp::eval(unsigned const nRes, FFVar* vRes, unsigned const nVar,
              FFVar const* vVar, unsigned const* mVar) const
{
...
inline void
FFNSamp::eval(unsigned const nRes, FFDep* vRes, unsigned const nVar,
              FFDep const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFNSAMP_TRACE
...
inline void
FFNSamp::eval(unsigned const nRes, double* vRes, unsigned const nVar,
              double const* vVar, unsigned const* mVar) const
{
#ifdef MC__FFNSAMP_TRACE
  ...
  if (_nc && (!_DCST || _DCST->size() < _nc))
    throw std::runtime_error(
        "FFNSamp::eval ** Constant values missing for constraint evaluation");
  if (_np &&
      (!_DPAR || !_WPAR || _DPAR->at(0).size() < _np || _WPAR->size() < _ns))
    throw std::runtime_error(
        "FFNSamp::eval ** Parameter values missing for constraint evaluation");
  ...
  {
    if (!_nc)
      _DAG->eval(_sgFCT, _wkD, _FFCT, _DFCT[0], _FCON, _DCON);
    else
      _DAG->eval(_sgFCT, _wkD, _FFCT, _DFCT[0], _FCON, _DCON, _FCST, *_DCST);
  }
  else if (_ns == 1)
  {
    if (!_nc)
      _DAG->eval(_sgFCT, _wkD, _FFCT, _DFCT[0], _FPAR, _DPAR->at(0), _FCON,
                 _DCON);
    else
      _DAG->eval(_sgFCT, _wkD, _FFCT, _DFCT[0], _FPAR, _DPAR->at(0), _FCON,
                 _DCON, _FCST, *_DCST);
  ...
  else
  {
    ...
    {
      if (!_nc)
        _DAG->eval(_sgFCT, _wkD, _FFCT, _DFCT[s], _FPAR, _DPAR->at(s), _FCON,
                   _DCON);
      else
        _DAG->eval(_sgFCT, _wkD, _FFCT, _DFCT[s], _FPAR, _DPAR->at(s), _FCON,
                   _DCON, _FCST, *_DCST);
...
#else
    if (!_nc)
      _DAG->veval(_sgFCT, _wkD, _wkThd, _FFCT, _DFCT, _FPAR, *_DPAR, _FCON,
                  _DCON);
    else
      _DAG->veval(_sgFCT, _wkD, _wkThd, _FFCT, _DFCT, _FPAR, *_DPAR, _FCON,
                  _DCON, _FCST, *_DCST);
  ...
  for (size_t s = 0; s < (_ns ? _ns : 1); ++s)
    std::cout << "FCT[" << s << "] = "
              << arma::rowvec(_DFCT[s].data(), _DFCT[s].size(), false);
#endif
...
inline void
FFNSamp::_feasval(double* vRes, std::vector<std::vector<double>>& DFCT) const
{
#ifdef MC__FFNSAMP_TRACE
  ...
  for (size_t s = 0; s < _ns; ++s)
  {
    ...
    ResCTR.insert(
        {-*itMax,
         _WPAR->at(s)});  // ordered by largest (negative) violation first
  }
...
#ifdef MC__FFNSAMP_DEBUG
  std::cout << "  Feas Pr = " << std::fixed << std::setprecision(1)
            << std::setw(5) << vRes[0] * 1e2
            << "  Feas VaR = " << std::scientific << std::setprecision(4)
            << std::setw(11) << vRes[1] << "  Feas CVaR = " << std::setw(11)
            << vRes[2] << std::endl;
#endif
}

inline void
FFNSamp::_lkhdval(double* vRes, std::vector<std::vector<double>>& DFCT) const
{
#ifdef MC__FFNSAMP_TRACE
  ...
  {
    ResLKH.insert(
        {DFCT[s][_ng], _WPAR->at(s)});  // ordered by smallest likelihood first
  }
