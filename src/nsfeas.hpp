...
////////////////////////////////////////////////////////////////////////
class NSFEAS : public virtual BASE_MBFA
{
 protected:
  typedef FFGraph DAG;
 ...
 public:
  ...
  struct Stats
  {
    ...
    display(std::ostream& os = std::cout)
    {
      os << std::fixed << std::setprecision(2) << std::right << std::endl
         ...
         << "** WALL TIME:   " << std::right << std::setw(10)
         << to_time(walltime) << " SEC" << std::endl;
    }
  ...
  int sample(std::vector<double> const& vcst = std::vector<double>(),
  ...
  */
  ...
  std::multimap<double, std::tuple<double const*, double, double>> const&
  live_points() const
  {
 ...
 protected:
  ...
  void _sample_bounds(std::ostream& os);

  //! @brief Sample current nest - return value: True if within bounds; False
  //! otherwise
  bool _sample_nest(bool const bndcheck, std::ostream& os);

  //! @brief Test for criterion feasibility
  bool _feasible(double const& crit, std::ostream& os) const;

  //! @brief Test for completion or interruption
  int _terminate(
      size_t const it, bool const feas,
      std::chrono::time_point<std::chrono::system_clock> const& tstart,
...
NSFEAS::setup()
{
  ...
  if (!_nu || (!_ng && _nl > 1)) throw Exceptions(Exceptions::BADSIZE);
    ...
    std::ostream& os)
{
  ...
  if (_vPARVAL.size() <= 1 && _dag->options.MAXTHREAD != 1)
  {
    ...
    for (size_t s = 0; s < _vVALSAM.size();
         ++s, stats.numfct += (_vPARVAL.size() > 0 ? _vPARVAL.size() : 1))
    {  // to account for failures
      if (!_dag->veval_err().at(s))
      {
        _liveFEAS.insert({DBL_MAX, {_vCONSAM[s].data(), 0., -DBL_MAX}});
#ifdef MAGNUS__NSFEAS_SAMPLE_DEBUG
        std::cout << "Failed scenario " << s << ":" << std::right
                  << std::scientific << std::setprecision(5)
                  << arma::rowvec(const_cast<double*>(_vCONSAM[s].data()), _nu,
                                  false);
#endif
        ++stats.numerr;
        continue;
      ...
      _liveFEAS.insert({_vVALSAM[s][options.FEASCRIT],
                        {_vCONSAM[s].data(), _vVALSAM[s][0],
                         _vVALSAM[s][2 + (int)options.LKHCRIT]}});
  ...
  {
    for (size_t s = 0; _liveFEAS.size() < NLIVE0 + nprop;
         ++s, stats.numfct += (_vPARVAL.size() > 0 ? _vPARVAL.size() : 1))
    {  // to account for failures
      // Sample new point
      _sample_bounds(os);
      ...
      try
      {
        ...
        _liveFEAS.insert(
            {_dVAL[options.FEASCRIT],
             {dCON.data(), _dVAL[0], _dVAL[2 + (int)options.LKHCRIT]}});
      ...
      if ((options.MAXCPU > 0 &&
           stats.to_time(stats.lapse(tstart)) >= options.MAXCPU) ||
    ...
    double const& factor, std::ostream& os)
{
  ...
  for (size_t i = 0; i < _nu; ++i)
  {
    std::cout << std::setw(13) << _nestLB(i) << std::setw(13) << _nestUB(i);
  }
  std::cout << std::endl;
#endif
  ...
  if (!arma::chol(_nestShape, covData, "lower"))
  {
    if (options.DISPLEVEL)
      std::cout << "Cholesky factorization failed, rank = "
                << arma::rank(covData) << std::endl;
  ...
  else
  {
    ...
    _nestData.each_col(
        ...
        {
          arma::vec x(_nu);
          arma::solve(x, arma::trimatl(_nestShape), v);
          covMag(c++) = arma::norm(x, 2);
        });
...
inline bool
NSFEAS::_sample_nest(bool const bndcheck, std::ostream& os)
{
  ...
  // arma::vec vSAM = arma::normalise( arma::randn(_nu), 2 ) * std::pow(
  // _gen1d(), 1/(double)_nu );
  arma::vec vSAM = arma::normalise(arma::randn(_nu), 2) *
                   std::pow(arma::randu(), 1 / (double)_nu);
#ifdef MAGNUS__NSFEAS_SAMPLE_DEBUG
  ...
  for (size_t i = 0; i < _nu && bndcheck; ++i)
    if (vSAM[i] < _vCONLB[i] || vSAM[i] > _vCONUB[i])
    {
      return false;
    }
...
inline bool
NSFEAS::_feasible(double const& crit, std::ostream& os) const
{
    ...
    std::ostream& os) const
{
  if (feas)
  {
    switch (options.FEASCRIT)
    {
      case Options::VAR:
      case Options::CVAR:
        if (_liveFEAS.rbegin()->first <= 0)
          return (1);  // completion for feasibility
    }
  }
  else
  {
    switch (options.LKHCRIT)
    {
      case Options::VAR:
      case Options::CVAR:
        if (_liveNest <= _totalMass + std::log(options.LKHTOL))
          return (1);  // completion for likelihood
    }
  }

  if (options.MAXCPU > 0. &&
      stats.to_time(stats.lapse(tstart)) < options.MAXCPU)
    return (2);                                                  // interruption
    ...
    std::ostream& os)
{
  ...
  {
    std::cout << std::endl
              << std::setw(7) << "Iterate" << std::setw(15) << "Contour"
              << std::setw(6) << "#Feas" << std::setw(9) << "#Dead"
              << std::setw(15) << "Factor" << std::endl
  ...
  for (stats.iter = 0; !flag; ++stats.iter)
  {
    ...
    if (!_update_nest(_liveFEAS, factor, os)) return (3);

    if (options.DISPLEVEL &&
        (!options.DISPITER || !(stats.iter % options.DISPITER)))
    {
      std::cout << std::setw(7) << stats.iter << std::scientific
                << std::setprecision(4) << std::setw(15)
                << _liveFEAS.rbegin()->first << std::setw(6) << stats.numfeas
                << std::setw(9) << _deadFEAS.size() << std::scientific
                << std::setprecision(4) << std::setw(15) << factor;
#ifdef MAGNUS__NSFEAS_SAMPLE_DEBUG
      std::cout << std::right << std::scientific << std::setprecision(5);
      for (size_t i = 0; i < _nu; ++i)
      {
        std::cout << std::setw(12) << _nestCentre(i) - _nestShape(i, i)
                  ...
                  << std::setw(12) << _nestLB(i) << std::setw(12) << _nestUB(i);
      }
#endif
      std::cout << std::endl;
    }
    ...
    if (_vPARVAL.size() <= 1 && _dag->options.MAXTHREAD != 1)
    {
      ...
      try
      {
        auto itSAM0 = _vCONSAM.begin();
        std::advance(itSAM0, NSAM0);
        //_vCONPROP.assign( itSAM0, _vCONSAM.end() );
        // Move elements to _vCONPROP to avoid unnecessary memory allocation
        _vCONPROP.assign(std::make_move_iterator(itSAM0),
                         std::make_move_iterator(_vCONSAM.end()));
#ifdef MAGNUS__NSFEAS_SAMPLE_DEBUG
        for (auto const& dCON : _vCONPROP)
          std::cout << std::right << std::scientific << std::setprecision(5)
                    << arma::rowvec(const_cast<double*>(dCON.data()), _nu,
                                    false);
#endif
        ...
        _vCONSAM.insert(_vCONSAM.end(),
                        std::make_move_iterator(_vCONPROP.begin()),
                        std::make_move_iterator(_vCONPROP.end()));
      }
      catch (...)
      {
        return FAILURE;
    ...
    else
    {
      ...
      for (size_t s = 0; s < options.NUMPROP; ++s)
      {
        ...
        std::cout << (_dVAL[options.FEASCRIT] <= _liveFEAS.rbegin()->first)
                  << std::right << std::scientific << std::setprecision(5)
                  << std::setw(12) << _dVAL[options.FEASCRIT]
...
#endif
#ifdef MAGNUS__NSFEAS_SAMPLE_DEBUG
        std::cout << "Failed " << std::right << std::scientific
                  << std::setprecision(5)
                  << arma::rowvec(const_cast<double*>(dCON.data()), _nu, false);
#endif
        ++stats.numerr;
  ...
  if (options.DISPLEVEL)
  {
    std::cout << std::setw(7) << stats.iter + 1 << std::scientific
              << std::setprecision(4) << std::setw(15)
              << _liveFEAS.rbegin()->first << std::setw(6) << stats.numfeas
              << std::setw(9) << _deadFEAS.size() << std::scientific
              << std::setprecision(4) << std::setw(15) << factor << std::endl;
  }
    ...
    std::ostream& os)
{
  if (options.DISPLEVEL)
  {
    std::cout << std::endl
              << std::setw(7) << "Iterate" << std::setw(15) << "Contour"
              << std::setw(9) << "#Dead" << std::setw(15) << "Mass"
              << std::setw(15) << "Nest" << std::setw(15) << "Factor"
              << std::endl
  ...
  for (; !flag; ++stats.iter)
  {
        ...
        (!options.DISPITER || !(stats.iter % options.DISPITER)))
    {
      std::cout << std::setw(7) << stats.iter << std::scientific
                << std::setprecision(4) << std::setw(15)
                << _liveLKH.begin()->first << std::setw(9) << _deadLKH.size()
                << std::scientific << std::setprecision(4) << std::setw(15)
                << _totalMass << std::scientific << std::setprecision(4)
                << std::setw(15) << _liveNest << std::scientific
                << std::setprecision(4) << std::setw(15) << factor << std::endl;
    }

    // Update ellipsoidal nest
    if (!_update_nest(_liveLKH, factor, os)) return (3);
         ...
         ++s, stats.numfct += (_vPARVAL.size() > 0 ? _vPARVAL.size() : 1))
    {
      ...
      _liveLKH.insert({_dVAL[2 + (int)options.LKHCRIT],
                       {dCON.data(), _dVAL[0], _dVAL[options.FEASCRIT]}});
      _liveLKH.erase(_liveLKH.begin());
      double nestMass =
          std::exp(-(double)_deadLKH.size() / (double)options.NUMLIVE);
      _deadShell = _liveLKH.begin()->first + std::log(_nestMass - nestMass);
#ifdef MAGNUS__NSFEAS_SAMPLE_DEBUG
      std::cout << "_totalMass: " << log_sum_exp(_totalMass, _deadShell)
                << " = " << _totalMass << " + "
                << log_sum_exp(_totalMass, _deadShell) - _totalMass
                << std::endl;
      assert(_totalMass <= log_sum_exp(_totalMass, _deadShell));
#endif
      _totalMass =
          (_nestMass < 1. ? log_sum_exp(_totalMass, _deadShell) : _deadShell);
      _liveNest = log_sum_exp(_liveLKH) - std::log(options.NUMLIVE) +
                  std::log(nestMass);
#ifdef MAGNUS__NSFEAS_SAMPLE_DEBUG
      std::cout << "_nestMass:  " << _nestMass << " >> " << nestMass
                << std::endl;
      std::cout << "_deadShell: " << _deadShell << " = "
                << _liveLKH.begin()->first << " + "
                << std::log(_nestMass - nestMass) << std::endl;
      std::cout << "_liveNest:  " << _liveNest << " = " << log_sum_exp(_liveLKH)
                << " - " << std::log(options.NUMLIVE) << " + "
                << std::log(nestMass) << std::endl;
      int dum;
  ...
  for (auto& [lkh, tup] : _deadLKH) std::get<2>(tup) -= _totalMass;

  if (options.DISPLEVEL)
    std::cout << std::setw(9 + 13) << std::setfill('-') << "-"
              << std::setfill(' ') << std::endl
              << "Evidence:" << std::scientific << std::setprecision(4)
              << std::setw(13) << _totalMass << std::endl;

  return flag;
}

inline int
NSFEAS::sample(std::vector<double> const& vcst, bool const reset,
               std::ostream& os)
{
  ...
  FFVar** ppVAL = OpEval(
      _dag, &_vCON, _vPAR.size() ? &_vPAR : nullptr,
      _vCST.size() ? &_vCST : nullptr, _vCTR.size() ? &_vCTR : nullptr,
      _vLKH.size() ? &_vLKH[0] : nullptr, _vPARVAL.size() ? &_vPARVAL : nullptr,
      _vPARWEI.size() ? &_vPARWEI : nullptr,
  ...
  if (reset)
  {
    ...
    if (options.DISPLEVEL)
      os << "(" << _liveFEAS.size() << (flag ? ")" : ") INTERRUPTED AT")
         ...
         << stats.to_time(stats.lapse(tstart)) << " SEC" << std::endl;
  ...
  {
    stats.numfeas = 0;
    for (auto const& [crit, pcon] : _liveFEAS)
    {  // to update feasible point count
  ...
  if (_nl)
  {
    ...
    for (auto const& [feas, pcon] : _liveFEAS)
      _liveLKH.insert(
          {std::get<2>(pcon), {std::get<0>(pcon), std::get<1>(pcon), feas}});
