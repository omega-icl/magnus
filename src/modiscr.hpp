...
////////////////////////////////////////////////////////////////////////
class MODISCR : public virtual BASE_MBDOE
{
 protected:
#if defined(MC__USE_PROFIL)
  typedef ::INTERVAL I;
 ...
 public:
  ...
  //! @brief Constructor
  MODISCR() : _dag(nullptr), _dagdoe(nullptr), _VOpt(0. / 0.) { stats.reset(); }
  ...
  std::vector<std::vector<std::vector<arma::vec>>> const&
  output_sample() const
  {
    return _vOUTSAM;
  }
...
MODISCR::setup()
{
  ...
  if (!_nm || !_ny) throw Exceptions(Exceptions::NOMODEL);
...
MODISCR::_setup_out()
{
  if (_nm != BASE_MBDOE::_vOUT.size() || _ny != BASE_MBDOE::_vOUT[0].size() ||
  ...
  _vOUT.resize(_nm);
  _vOUTVEC.clear();
  _vOUTVEC.reserve(_nm * _ny);
  _sgOUT.resize(_nm);
  for (size_t m = 0; m < _nm; ++m)
  {
    _vOUT[m].resize(_ny);
    _dag->insert(BASE_MBDOE::_dag, _ny, BASE_MBDOE::_vOUT[m].data(),
                 _vOUT[m].data());
    _sgOUT[m].clear();
...
inline bool
MODISCR::sample_support(size_t const NSAM, std::vector<double> const& vcst,
                        std::ostream& os)
{
...
inline bool
MODISCR::_sample_out(size_t const NSAM, std::ostream& os)
{
  ...
  {
    if (!_append_out(_vCONAP[e], _vPARVAL, _dOUT, _vOUTSAM, os)) return false;
    if (options.DISPLEVEL && !(e % DISPFREQ)) os << "=" << std::flush;
  }

  // Compute responses for every control samples and uncertainty scenarios
  for (size_t e = 0; e < NSAM; ++e)
  {
    if (!_append_out(_vCONSAM[e], _vPARVAL, _dOUT, _vOUTSAM, os)) return false;
    if (options.DISPLEVEL && !((e + _ne0) % DISPFREQ)) os << "=" << std::flush;
  }
  if (options.DISPLEVEL)
...
inline bool
MODISCR::_append_out(std::vector<double> const& Control,
                     std::vector<std::vector<double>> const& Parameter,
                     std::vector<std::vector<double>>& Output,
                     std::vector<std::vector<std::vector<arma::vec>>>& Response,
                     std::ostream& os)
{
  ...
  for (size_t m = 0; m < _nm; ++m)
  {
    // Compute Output of model m for Control in all uncertainty scenarios
    // Parameter
    try
    {
      if (_nc)
        _dag->veval(_sgOUT[m], _wkOUT, _vOUT[m], Output, _vPAR, Parameter,
                    _vCON, Control, _vCST, _vCSTVAL);
      else
        _dag->veval(_sgOUT[m], _wkOUT, _vOUT[m], Output, _vPAR, Parameter,
                    _vCON, Control);
    }
    ...
    for (size_t s = 0; s < Output.size(); ++s)
    {
      ...
      if (Response[s][m].size() &&
          arma::size(Response[s][m].back()) != arma::size(vOut))
        throw Exceptions(Exceptions::BADSIZE);
      Response[s][m].push_back(vOut);
#ifdef MAGNUS__MODISCR_SAMPLE_DEBUG
      std::cout << "OUT[" << s << "][" << m << "][" << Response[s][m].size()
                << "]:" << std::endl
                << arma::trans(vOut);
#endif
    }
...
inline size_t
MODISCR::_redundant_support(std::vector<double> const& suppref)
{
  size_t pos = 0;
...
inline double
MODISCR::_mae_support(std::vector<double> const& s1,
                      std::vector<double> const& s2)
{
...
inline bool
MODISCR::_update_supports(std::map<size_t, double> const EOpt,
                          std::map<size_t, std::vector<double>> const SOpt,
                          std::ostream& os)
{
  auto&& t_samgen = stats.start();
...
inline bool
MODISCR::file_export(std::string const& name)
{
  ...
  for (size_t s = 0; s < _vPARVAL.size(); ++s, ++itPARVAL)
  {
    ...
    for (size_t k = 0; k < _vCONSAM.size(); ++k)
    {
      ...
      for (size_t m = 0; m < _vOUTSAM[s].size(); ++m) switch (options.CRITERION)
        {
          case BRISK:
...
inline int
MODISCR::combined_solve(size_t const NEXP, std::vector<double> const& vcst,
...
inline void
MODISCR::_effort_set_BRNeutral(MINLP& doe, size_t const NEXP,
                               std::vector<FFVar> const& EFF) const
{
  FFLin<I> Sum;
...
inline void
MODISCR::_effort_set_BRAverse(MINLP& doe, size_t const NEXP,
                              std::vector<FFVar> const& EFF,
                              std::vector<double>& E0) const
{
  FFLin<I> Sum;
...
inline int
MODISCR::effort_solve(size_t const NEXP, bool const exact,
...
inline double
MODISCR::_evaluate_BRNeutral(std::map<size_t, double> const& EOpt,
                             std::vector<FFVar> const& UTOT,
                             std::vector<double> const& UTOT0, std::ostream& os)
{
  ...
  FFBRMMCrit OpBR;
  FFVar const* const* ppBR =
      OpBR(UTOT.data(), _dag, _nm, &_vPAR, &_vCST, &_vCON, &_vOUTVEC, &EOpt,
  ...
  for (size_t i = 0; i < UTOT.size(); i++)
  {
    ...
    double DBR_plus;
    _dagdoe->eval(1, &FBR, &DBR_plus, UTOT.size(), UTOT.data(),
                  UTOT0_pert.data());
    UTOT0_pert[i] = UTOT0[i] * (1 - RTOL) - ATOL;
    double DBR_minus;
    _dagdoe->eval(1, &FBR, &DBR_minus, UTOT.size(), UTOT.data(),
                  UTOT0_pert.data());
    std::cout << std::scientific << std::setprecision(5);
    std::cout << "d_UTOT[" << i << "] = " << 2 * (UTOT0[i] - UTOT0_pert[i])
              << std::endl;
    std::cout << "grad BR[" << i << "] = "
              << (DBR_plus - DBR_minus) / (2 * (UTOT0[i] - UTOT0_pert[i]))
              << "  <?>  " << DGRADBR[i] << std::endl;
  }
#endif
...
inline double
MODISCR::_evaluate_BRAverse(std::map<size_t, double> const& EOpt,
                            std::vector<FFVar> const& UTOT,
                            std::vector<double> const& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  ...
  for (size_t s = 0; s < NUNC; ++s) FBR[s] = *ppBR[s];

  // Evaluate cost function
  _dagdoe->eval(FBR, DBR, UTOT, UTOT0);

  std::multimap<double, double> SA;
  for (size_t s = 0; s < NUNC; ++s) SA.insert({DBR[s], _vPARWEI[s]});
    ...
    std::string const& type, std::vector<double> const& vcst, std::ostream& os)
{
  ...
  {
    if (options.DISPLEVEL)
      _display_design(header, 0. / 0.,
                      std::list<std::pair<double, std::vector<double>>>(), os);
    return std::make_pair(0. / 0., false);  // NaN
  }

  if (options.DISPLEVEL) _display_design(header, crit, Campaign, os);
  return std::make_pair(crit, true);
}

inline void
MODISCR::_refine_set_BRNeutral(NLP& doeref,
                               ...
                               std::ostream& os)
{
  ...
  std::vector<double> DGRADBR;
  _dagdoe->eval(FGRADBR, DGRADBR, UTOT, UTOT0);
  std::cout << "Grad Cit AD = "
            << arma::trans(arma::vec(DGRADBR.data(), DGRADBR.size(), false))
            << std::endl;
...
inline void
MODISCR::_refine_set_BRAverse(NLP& doeref, std::map<size_t, double> const& EOpt,
                              std::vector<FFVar> const& UTOT,
                              std::vector<double>& UTOT0, std::ostream& os)
{
  // Check supports for a priori experiments
  ...
  FFLin<I> Sum;
  doeref.set_obj(BASE_OPT::MIN, VaR + Sum(NUNC, DELTA.data(), _vPARWEI.data()) /
                                          options.CVARTHRES);
  for (size_t s = 0; s < NUNC; s++)
...
inline int
MODISCR::gradient_solve(std::map<size_t, double> const& EOpt,
...
inline void
MODISCR::_display_design(std::string const& title, double const& crit,
                         std::map<size_t, double> const& eff,
                         std::map<size_t, std::vector<double>> const& supp,
                         std::ostream& os) const
{
