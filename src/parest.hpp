...
////////////////////////////////////////////////////////////////////////
class PAREST : public virtual BASE_PAREST
{
 protected:
  typedef FFGraph DAG;
 ...
 private:
  ...
  //! @brief local copy of model constraints (constraint lhs, type, constraint
  //! rhs)
 ...
 public:
  ...
  //! @brief Constructor
  PAREST() : _dag(nullptr), _MLECrit(0.) { stats.reset(); }

  //! @brief Destructor
  virtual ~PAREST() { delete _dag; }

  //! @brief PAREST solver options
  struct Options
  {
    //! @brief Constructor
    Options() : NLPSLV(), NSSLV() { reset(); }
    ...
    int DISPLEVEL;
    //! @brief Random-number generator seed. =0: Sobol sampling; >0: seed set to
    //! specified value; <0: seed set to a value drawn from std::random_device
    int RNGSEED;
  ...
  struct Stats
  {
    ...
    display(std::ostream& os = std::cout)
    {
      os << std::fixed << std::setprecision(2) << std::right << std::endl
         ...
         << to_time(walltime_slvmle) << " SEC" << std::endl
         << "#  NESTED SAMPLING SOLVE: " << std::setw(10)
         << to_time(walltime_slvns) << " SEC" << std::endl
         << "#  TOTAL:                 " << std::setw(10)
    ...
    //! @brief Cumulated wall-clock time used by nested sampling solver (in
    //! microseconds)
  ...
  void bootstrap_sample(std::vector<std::vector<Experiment>> const& data,
                        size_t const nsam, std::ostream& os = std::cout);

  //! @brief Sample confidence region using bootstrapping around given data set
  void bootstrap_sample(std::vector<Experiment> const& data, size_t const nsam,
                        std::ostream& os = std::cout);

  //! @brief Sample confidence region using bootstrapping around best parameter
  //! estimates
  void bootstrap_sample(size_t const nsam, std::ostream& os = std::cout);

  //! @brief Return covariance from sampled parameter posterior
  arma::mat cov_sample(std::ostream& os = std::cout) const;

  //! @brief Return covariance for given parameter sample
  arma::mat cov_sample(arma::mat const& PARSam,
                       std::ostream& os = std::cout) const;

  //! @brief Compute covariance matrix using linearization (Wald)
  arma::mat cov_linearized(std::ostream& os = std::cout);

  //! @brief Compute parameter confidence intervals
  arma::vec conf_interval(arma::mat const& covmat,
                          double const& conflevel = 0.95,
                          std::string const& type = "T",
                          std::ostream& os        = std::cout);

  //! @brief Sample parameter (i,j) confidence ellipsoid
  arma::mat conf_ellipsoid(arma::mat const& covmat, size_t const i,
                           size_t const j, double const& conflevel = 0.95,
                           std::string const& type = "F",
                           size_t const nsam       = 50,
                           std::ostream& os        = std::cout);

  //! @brief Return weighted samples in level*100% HPD region
  std::pair<arma::mat, arma::vec> hpd_region(
      double const& level = 0.95, std::ostream& os = std::cout) const;
  ...
  arma::vec hpd_quantile(double const& level,
                         std::ostream& os = std::cout) const;

  //! @brief Return min range of level*100% HPD region
  arma::vec hpd_min(double const& level, std::ostream& os = std::cout) const;

  //! @brief Return max range of level*100% HPD region
  arma::vec hpd_max(double const& level, std::ostream& os = std::cout) const;

  //! @brief Return mean of parameter posterior
  arma::vec hpd_mean(std::ostream& os = std::cout) const;
 ...
 private:
  //! @brief Compute chi-squared (good-of-fit) test
  std::tuple<double, double, double> _chi2_test(double const& conf,
                                                std::vector<double> const& P0,
                                                std::vector<double> const& C0,
                                                int const disp,
                                                std::ostream& os);

  //! @brief Return position of last weighted samples in HPD region at
  //! confidence level conf*100%
  size_t _pos(double const& level, arma::vec const& wei) const;

  //! @brief Return mean of weighted samples in log space
  double _mean(arma::vec const& par, arma::vec const& wei) const;

  //! @brief Return quantile at confidence level conf*100% in 1d weighted sample
  double _quantile(double const& level, arma::vec const& val,
                   arma::vec const& wei) const;

  //! @brief Return conf*100% quantile of parameter posterior
  arma::vec _hpd_quantile(double const& level) const;
};
...
PAREST::setup()
{
  ...
  for (size_t m = 0; m < _nm; nytot += _ny[m++])
  {
    if (_nu[m])
    {
      ...
      _dag->insert(BASE_PAREST::_dag, _nu[m], BASE_PAREST::_vCON[m].data(),
                   _vCON[m].data());
    }
    if (_ny[m])
    {
      _vOUT[m].resize(_ny[m]);
      _dag->insert(BASE_PAREST::_dag, _ny[m], BASE_PAREST::_vOUT[m].data(),
                   _vOUT[m].data());
  ...
  _vREG.resize(_nr);
  if (_nr)
    _dag->insert(BASE_PAREST::_dag, _nr, BASE_PAREST::_vREG.data(),
                 _vREG.data());
  ...
  if (_ng)
  {
    _dag->insert(BASE_PAREST::_dag, _ng, std::get<0>(BASE_PAREST::_vCTR).data(),
                 std::get<0>(_vCTR).data());
    _dag->insert(BASE_PAREST::_dag, _ng, std::get<2>(BASE_PAREST::_vCTR).data(),
                 std::get<2>(_vCTR).data());
  }
...
inline int
PAREST::sme_solve(double const& conf, std::vector<double> const& C0,
                  std::ostream& os)
{
  _WEISam.reset();
  _PARSam.reset();
  ...
  {
    if (!_ny[m]) continue;
    _MLECrit += OpMLE(_vPAR.data(), _dag, _vPAR, _vCST, _vCON[m], _vOUT[m],
                      &_vCSTVAL, &_vDAT[m]);
  }
  ...
  FFLin<I> OpSum;
  if (_nr)
    PE.add_constraint(2 * _MLECrit + OpSum(_vREG, 1.), LE, Chi2Crit);
  else
    PE.add_constraint(2 * _MLECrit, LE, Chi2Crit);

  // Other constraints
  for (size_t g = 0; g < _ng; ++g)
    PE.add_constraint(std::get<0>(_vCTR).at(g), std::get<1>(_vCTR).at(g),
                      std::get<2>(_vCTR).at(g));

  PE.setup();
  int iflag = PE.sample();

  if (options.NSSLV.DISPLEVEL) PE.stats.display();
  ...
  if (iflag == PE.NORMAL)
  {
    ...
    if (options.DISPLEVEL)
    {
      ...
      os << "GOODNESS-OF-FIT: " << std::fixed << std::setprecision(0)
         << _PAREst.f[0] * 1e2 << "% CONFIDENCE LEVEL" << std::setw(13)
         << std::endl;
    }

    if (options.DISPLEVEL) os << "\n# SAMPLED ADEQUACY REGION:\n";

    size_t isam = 0;
    _PARSam.set_size(PE.live_points().size(), _np);
    for (auto it = PE.live_points().cbegin(); it != PE.live_points().cend();
         ++it, ++isam)
    {
      auto const& [feas, tup] = *it;
      _PARSam.row(isam) =
          arma::mat(const_cast<double*>(std::get<0>(tup)), 1, _np, false);
      if (options.DISPLEVEL > 1)
      {
        os << std::setw(5) << std::right << isam << ": " << std::scientific
           << std::setprecision(6) << std::setw(14) << feas << " | ";
        auto vecx =
            arma::vec(const_cast<double*>(std::get<0>(tup)), _np, false);
        vecx.t().raw_print(os);
      }
    }
...
inline int
PAREST::bpe_solve(std::vector<double> const& C0, std::ostream& os)
{
  _WEISam.reset();
  _PARSam.reset();
  _PAREst.reset();
  ...
  FFLin<I> OpSum;
  if (_nr)
    PE.set_loglikelihood(_MLECrit - OpSum(_vREG, 1.));
  else
    PE.set_loglikelihood(_MLECrit);
  ...
  if (iflag == PE.NORMAL)
  {
         ...
         ++it, ++isam)
    {
      ...
      WEItot        = (it != PE.live_points().crbegin()
                           ? log_sum_exp(WEItot, std::get<2>(tup))
                           : std::get<2>(tup));
      if (options.DISPLEVEL > 1)
      {
        os << std::setw(5) << std::right << isam << ": " << std::scientific
           << std::setprecision(6) << std::setw(14) << lkh << " | "
           << std::setw(14) << std::get<2>(tup) << " | " << std::setw(14)
           << WEItot << " | ";
        auto vecx =
            arma::vec(const_cast<double*>(std::get<0>(tup)), _np, false);
        vecx.t().raw_print(os);
    ...
    for (auto it = PE.dead_points().crbegin(); it != PE.dead_points().crend();
         ++it, ++isam)
    {
      auto const& [lkh, tup] = *it;
      _PARSam.row(isam) =
...
inline size_t
PAREST::_pos(double const& level, arma::vec const& wei) const
{
  ...
  for (; mass < std::log(level);)
  {
    ...
    std::cout << "pos: " << pos << "  mass: " << mass
              << "  threshold: " << std::log(level) << std::endl;
#endif
...
inline std::pair<arma::mat, arma::vec>
PAREST::hpd_region(double const& level, std::ostream& os) const
{
  ...
  {
    os << std::endl
       << "# " << std::fixed << std::setprecision(0) << level * 1e2
...
inline arma::vec
PAREST::hpd_min(double const& level, std::ostream& os) const
{
  // compute cumulated probability mass _WEISam up to level
  ...
  {
    os << std::endl
       << "# MINIMUM RANGE OF " << std::fixed << std::setprecision(0)
       << level * 1e2 << "% HPD PARAMETER REGION:\n"
       << std::scientific << std::setprecision(5);
    PARMin.raw_print(os);
  }
...
inline arma::vec
PAREST::hpd_max(double const& level, std::ostream& os) const
{
  // compute cumulated probability mass _WEISam up to level
  if (_WEISam.empty()) return arma::vec();
  size_t pos = _pos(level, _WEISam);
  ...
  {
    os << std::endl
       << "# MAXIMUM RANGE OF " << std::fixed << std::setprecision(0)
       << level * 1e2 << "% HPD PARAMETER REGION:\n"
       << std::scientific << std::setprecision(5);
    PARMax.raw_print(os);
  }
...
inline double
PAREST::_quantile(double const& level, arma::vec const& val,
                  arma::vec const& wei) const
{
...
inline arma::vec
PAREST::_hpd_quantile(double const& level) const
{
  ...
  size_t p = 0;
  _PARSam.each_col([&](arma::vec const& v)
...
inline arma::vec
PAREST::hpd_quantile(double const& level, std::ostream& os) const
{
  ...
  {
    os << std::endl
       << "# " << std::fixed << std::setprecision(1) << level * 1e2
...
inline arma::vec
PAREST::hpd_mean(std::ostream& os) const
{
  if (_PARSam.empty()) return arma::vec();
  assert(_PARSam.n_cols == _np);
  ...
  size_t p = 0;
  _PARSam.each_col([&](arma::vec const& par)
...
inline double
PAREST::_mean(arma::vec const& par, arma::vec const& wei) const
{
  ...
  ;
  double mean =
      (wei.n_rows ? wei(0) + std::log(par(0)) : std::log(par(0) / par.n_rows));
  for (size_t pos = 0;;)
  {
    ++pos;
    if (pos == par.n_rows) break;
    if (wei.n_rows)
      mean = log_sum_exp(mean, wei(pos) + std::log(par(pos)));
    else
      mean = log_sum_exp(mean, std::log(par(pos) / par.n_rows));
  }

  return std::exp(mean);
}

inline std::pair<arma::vec, arma::vec>
PAREST::hpd_interval(double const& level, std::ostream& os) const
{
...
inline int
PAREST::mle_solve(std::vector<double> const& P0, std::vector<double> const& C0,
                  std::ostream& os)
{
  _PAREst.reset();
  ...
  for (size_t g = 0; g < _ng; ++g)
    PE.add_ctr(std::get<1>(_vCTR).at(g),
               std::get<0>(_vCTR).at(g) - std::get<2>(_vCTR).at(g));
...
inline int
PAREST::mle_solve(size_t const nsam, std::vector<double> const& C0,
                  std::ostream& os)
{
  _PAREst.reset();
  ...
  PE.setup();
  int iflag = PE.solve(nsam, _vPARLB.data(), _vPARUB.data(), _vCSTVAL.data(),
                       nullptr, 1);
...
PAREST::chi2_test(double const& conf, std::ostream& os)
{
  if (!_PAREst.x.empty())
...
inline std::tuple<double, double, double>
PAREST::chi2_test(double const& conf, std::vector<double> const& P0,
...
inline std::tuple<double, double, double>
PAREST::_chi2_test(double const& conf, std::vector<double> const& P0,
                   std::vector<double> const& C0, int const disp,
                   std::ostream& os)
{
  ...
  try
  {
    _dag->eval(_wkOUT, 1, &_MLECrit, &Chi2Val, _np, _vPAR.data(), P0.data(),
               _nc, _vCST.data(), _vCSTVAL.data());
    Chi2Val *= 2;
  }
  catch (...)
  {
  ...
  if (disp > 0)
  {
    os << "\n# CHI-SQUARED TEST: " << std::scientific << std::setprecision(3)
       << Chi2Val << (Chi2Val < Chi2Crit ? " < " : " > ") << Chi2Crit
       << " CRITICAL CHI_SQUARED VALUE (" << std::fixed << std::setprecision(0)
       << conf * 1e2 << "%, " << _nd - _np << " DOF)" << std::endl;
  }

  double Chi2Conf = cdf(dist, Chi2Val);
  if (disp > 0)
    os << "# CHI-SQUARED TEST PASSED WITH >" << std::fixed
       << std::setprecision(0) << Chi2Conf * 1e2 << "% CONFIDENCE LEVEL"
       << std::endl;
...
inline void
PAREST::bootstrap_sample(size_t const nsam, std::ostream& os)
{
  _WEISam.reset();
  _PARSam.reset();
  ...
  {
    if (!_ny[m]) continue;
    vMLECrit[m] = OpMLE(_vPAR.data(), _dag, _vPAR, _vCST, _vCON[m], _vOUT[m],
                        &_PAREst.p, &_vDAT[m]);
    _MLECrit += vMLECrit[m];
  }
  ...
  try
  {
    _dag->eval(_wkOUT, 1, &_MLECrit, &MLERes, _np, _vPAR.data(),
               _PAREst.x.data(), _nc, _vCST.data(), _PAREst.p.data());
  }
  catch (...)
  {
    return;
  ...
  for (size_t m = 0, e = 0; m < _nm; ++m, e = 0)
  {
    if (!_ny[m]) continue;
    auto const& dOUT =
        dynamic_cast<FFMLE const*>(vMLECrit[m].opdef().first)->dOUT();
    for (auto& EXP : vDAT0[m])
    {
      for (auto& [k, RECk] : EXP.output)
        for (auto& YMk : RECk.measurement) YMk = dOUT.at(e).at(k);
      ++e;
    }
  }
...
inline void
PAREST::bootstrap_sample(std::vector<Experiment> const& data, size_t const nsam,
                         std::ostream& os)
{
  std::vector<std::vector<Experiment>> vDAT0;
  for (auto const& exp : data) _add_data(vDAT0, exp);

  return bootstrap_sample(vDAT0, nsam, os);
}

inline void
PAREST::bootstrap_sample(std::vector<std::vector<Experiment>> const& data,
                         size_t const nsam, std::ostream& os)
{
  ...
  for (size_t m = 0; m < _nm; ++m)
    for (auto& EXP : data[m])
      for (auto& [k, RECk] : EXP.output) nd += RECk.measurement.size();
  ...
  for (size_t isam = 0; isam < nsam; ++isam)
  {
        ...
        for (auto& [k, RECk] : EXP.output)
        {
          ...
          for (auto& YMk : RECk.measurement)
          {
            double const dY =
                (options.RNGSEED
                     ? arma::randn(
                           arma::distr_param(0., std::sqrt(RECk.variance)))
                     : quantile(normal(0., std::sqrt(RECk.variance)), noise()));
            YMk += dY;
#ifdef MAGNUS__PAREST_CONF_DEBUG
            std::cout << "YM(" << isam << ")[" << std::to_string(m) << ","
                      << std::to_string(e) << "][" << std::to_string(k) << "]["
                      << std::to_string(r++) << "] = " << YMk << std::endl;
#endif
            lkh   = (first ? arma::log_normpdf(dY, 0., std::sqrt(RECk.variance))
                           : log_sum_exp(lkh,
                                         arma::log_normpdf(
                                           dY, 0., std::sqrt(RECk.variance))));
            first = false;
#ifdef MAGNUS__PAREST_CONF_DEBUG
            std::cout << dY << "  "
                      << arma::normpdf(dY, 0., std::sqrt(RECk.variance)) << "  "
                      << arma::log_normpdf(dY, 0., std::sqrt(RECk.variance))
                      << "  " << lkh << std::endl;
#endif
          }
        }
    ...
    // Update MLE objective - could also use set_obj_lazy - and solve from MLE
    // estimate
    FFMLE OpMLE;
    _MLECrit = 0.;
    for (size_t m = 0; m < _nm; ++m)
         ...
         PE.is_feasible(options.NLPSLV.FEASTOL)))
    {
      ...
      //_PARSam.insert_rows( _PARSam.n_rows, arma::mat(
      // const_cast<double*>(PE.solution().x.data()), 1, _np, false ) );
      if (options.DISPLEVEL > 1)
      {
        os << std::setw(5) << std::right << isam << ": " << std::scientific
           << std::setprecision(5) << std::setw(12) << PE.solution().f[0]
           << " | ";
...
inline arma::mat
PAREST::cov_sample(std::ostream& os) const
{
  return cov_sample(_PARSam, os);
}

inline arma::mat
PAREST::cov_sample(arma::mat const& PARSam, std::ostream& os) const
{
...
inline arma::mat
PAREST::cov_linearized(std::ostream& os)
{
  if (NLP::get_status(_PAREst.stat) != NLP::SUCCESSFUL &&
      NLP::get_status(_PAREst.stat) != NLP::FAILURE)
    return arma::mat();
  ...
  dagmle.insert(BASE_PAREST::_dag, _nc, BASE_PAREST::_vCST.data(),
                vCSTMLE.data());
  std::vector<mc::FFVar> vPARMLE(_np);
  dagmle.insert(BASE_PAREST::_dag, _np, BASE_PAREST::_vPAR.data(),
                vPARMLE.data());
  ...
  for (size_t m = 0, e = 0, d = 0; m < _nm; ++m, e = 0)
  {
    ...
    if (_nu[m])
      dagmle.insert(BASE_PAREST::_dag, _nu[m], BASE_PAREST::_vCON[m].data(),
                    vCONMLE.data());
    std::vector<mc::FFVar> vOUTMLE(_ny[m]);
    dagmle.insert(BASE_PAREST::_dag, _ny[m], BASE_PAREST::_vOUT[m].data(),
                  vOUTMLE.data());
    ...
    for (auto& EXP : _vDAT[m])
    {
      ...
      {
        mc::FFVar UMLE_e_u(&dagmle, "U[" + std::to_string(m) + "," +
                                        std::to_string(e) + "][" +
                                        std::to_string(u) + "]");
      ...
      std::vector<mc::FFVar> vYMLE_e =
          dagmle.compose(vOUTMLE, vCONMLE, vUMLE_e);
      for (auto& [k, RECk] : EXP.output)
      {
        for (unsigned r = 0; r < RECk.measurement.size(); ++r, ++d)
        {
          mc::FFVar YMMLE_e_k_r(&dagmle, "YM[" + std::to_string(m) + "," +
                                             std::to_string(e) + "][" +
                                             std::to_string(k) + "][" +
                                             std::to_string(r) + "]");
        ...
        dYMMLE.insert(dYMMLE.end(), RECk.measurement.cbegin(),
                      RECk.measurement.cend());
      }
      ++e;
    }
  }
  ...
  if (_nr)
  {
    dagmle.insert(BASE_PAREST::_dag, _nr, BASE_PAREST::_vREG.data(),
                  vREGMLE.data());
    FFLin<I> OpSum;
  ...
  for (size_t g = 0; g < _ng; ++g)
    vCTR[g] =
        std::get<0>(BASE_PAREST::_vCTR)[g] - std::get<2>(BASE_PAREST::_vCTR)[g];
  ...
  for (size_t p = 0; p < _np; ++p)
  {
    if ((std::fabs(_PAREst.x[p] - _vPARLB[p]) > options.NLPSLV.FEASTOL) &&
        (std::fabs(_PAREst.x[p] - _vPARUB[p]) > options.NLPSLV.FEASTOL))
      continue;
    vMULMLE.push_back(mc::FFVar(&dagmle, "MP[" + std::to_string(p) + "]"));
    dMULMLE.push_back(_PAREst.ux[p]);
    FMLE += vPARMLE[p] *
            vMULMLE[na++];  // omit bound constant since to be differentiated
  }
  ...
  auto pDFMLE = dagmle.FAD(1, &FMLE, _np, vPARMLE.data(), na, vMULMLE.data(),
                           _nd, vYMMLE.data());
#ifdef MAGNUS__PAREST_CONF_DEBUG
  ...
  dagmle.eval(_np + na, pDFMLE, dDFMLE.data(), _np, vPARMLE.data(),
              _PAREst.x.data(), vUMLE.size(), vUMLE.data(), dUMLE.data(), _nd,
              vYMMLE.data(), dYMMLE.data(), _nc, vCSTMLE.data(),
              _PAREst.p.data(), na, vMULMLE.data(), dMULMLE.data());
  ...
  auto tD2FMLE    = dagmle.SFAD(_np + na + _nd, pDFMLE, _np, _vPAR.data(), na,
                                vMULMLE.data());
  ...
  for (unsigned k = 0; k < ne; ++k)
    std::cout << "D2FMLE(" << std::get<1>(tD2FMLE)[k] << ","
              << std::get<2>(tD2FMLE)[k] << ") = " << exD2FMLE[k] << std::endl;
#endif

  dagmle.eval(ne, std::get<3>(tD2FMLE), dD2FMLE.data(), _np, vPARMLE.data(),
              _PAREst.x.data(), vUMLE.size(), vUMLE.data(), dUMLE.data(), _nd,
              vYMMLE.data(), dYMMLE.data(), _nc, vCSTMLE.data(),
              _PAREst.p.data(), na, vMULMLE.data(), dMULMLE.data());
  ...
  for (unsigned k = 0; k < ne; ++k)
  {
    ...
    if (i < _np + na)
      mD2FMLEDP2(i, j) = dD2FMLE[k];
    else
      mD2FMLEDYDP(i - _np - na, j) = dD2FMLE[k];
  }
#ifdef MAGNUS__PAREST_CONF_DEBUG
  std::cout << "d2FMLE/dP2 =\n" << mD2FMLEDP2;
...
inline arma::vec
PAREST::conf_interval(arma::mat const& covmat, double const& conflevel,
                      std::string const& type, std::ostream& os)
{
#ifdef MAGNUS__PAREST_CONF_DEBUG
  ...
  {
    os << std::endl
       << "# " << std::fixed << std::setprecision(0) << conflevel * 1e2
...
inline arma::mat
PAREST::conf_ellipsoid(arma::mat const& covmat, size_t const i, size_t const j,
                       ...
                       size_t const nsam, std::ostream& os)
{
  ...
  else if (type == "F")
  {
    ...
    try
    {
      _dag->eval(_wkOUT, 1, &_MLECrit, &rhs, _np, _vPAR.data(),
                 _PAREst.x.data(), _nc, _vCST.data(), _PAREst.p.data());
      rhs *= 2. * _np / double(_nd - _np) *
             quantile(fisher_f(_np, _nd - _np), conflevel);
    }
    catch (...)
    {
      return CRSam;
  ...
  arma::rowvec range =
      arma::linspace<arma::rowvec>(0, 2 * arma::datum::pi, nsam);
  ...
  std::cout << arma::sqrtmat_sympd(covmat.submat(ij, ij)) *
                   arma::sqrtmat_sympd(covmat.submat(ij, ij));
#endif

  CRSam =
      points.t() * arma::sqrtmat_sympd(covmat.submat(ij, ij)) * std::sqrt(rhs);
  CRSam += arma::kron(arma::conv_to<arma::mat>::from(
                          arma::vec(_PAREst.x.data(), _np, false))
                          .rows(ij),
                      arma::mat(1, nsam, arma::fill::ones))
               .t();
  if (options.DISPLEVEL)
  {
    os << std::endl
...
// IMPLEMENT LIKELIHOOD RATIO CONFIDENCE REGION WHEN NESTED SAMPLING AVAILABLE
// IN MAGNUS
