...
namespace mc
{
//! @brief C++ base class for defining of model-based feasibility analysis
//! problems
////////////////////////////////////////////////////////////////////////
...
//! problems
////////////////////////////////////////////////////////////////////////
class BASE_MBFA : public virtual BASE_OPT, public virtual BASE_SAMPLING
{
 protected:
 ...
 public:
  ...
  BASE_MBFA() : _dag(nullptr), _ng(0), _nl(0), _np(0), _nc(0), _nu(0) {}
                ...
                std::list<std::vector<double>> const& l_valP)
  {
    ...
    _vPARWEI.assign(_vPARVAL.size(),
                    1 / (double)l_valP.size());  // equal frequencies
  }
  ...
  void
  set_control(std::vector<FFVar> const& C, std::vector<double> const& CLB,
              std::vector<double> const& CUB)
  {
  ...
  void
  set_constraint(std::vector<FFVar> const& G)
  {
    assert(!G.empty());
    _ng   = G.size();
    _vCTR = G;
  ...
  void
  add_constraint(std::vector<FFVar> const& G)
  {
    if (G.empty()) return;
    _ng += G.size();
  ...
  void
  set_constraint(FFVar const& lhs, t_CTR const type = LE,
  ...
  void
  add_constraint(FFVar const& lhs, t_CTR const type = LE,
                 FFVar const& rhs = FFVar(0.))
  {
    assert(type != EQ);
    _ng++;
    if (type == LE)
      _vCTR.push_back(lhs - rhs);
    else
      _vCTR.push_back(rhs - lhs);
  }
  ...
  void
  set_loglikelihood(FFVar const& LL)
  {
    _nl   = 1;
    _vLKH = {LL};
  }
