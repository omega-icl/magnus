...
namespace mc
{
//! @brief C++ base class for defining of model-based design of experiment
//! problems
////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////
class BASE_MBDOE : public virtual BASE_MBFA
{
 protected:
 ...
 public:
  ...
  BASE_MBDOE() : BASE_MBFA(), _nm(0), _ny(0), _ne0(0) {}
  ...
  void
  set_model(std::vector<FFVar> const& Y,
  ...
  void
  set_model(std::list<std::vector<FFVar>> const& l_Y,
            std::vector<double> const& varY = std::vector<double>())
  {
  ...
  //! @brief Set model outputs for multiple candidate models
  void
  set_model(std::list<std::pair<std::vector<FFVar>, double>> const& l_Y,
            std::vector<double> const& varY = std::vector<double>())
  {
    ...
    _vOUTWEI.clear();
    _vOUTWEI.reserve(_nm);
    for (auto const& [Ym, Wm] : l_Y) _vOUTWEI.push_back(Wm / Wtot);
  ...
  void
  set_parameter(std::vector<FFVar> const& P, std::vector<double> const& valP,
                arma::mat const& scaP)
  {
  ...
  //! @brief Set list of model parameter scenarios and parameter scaling (matrix
  //! format)
  void
  set_parameter(std::vector<FFVar> const& P,
                std::list<std::vector<double>> const& l_valP,
                arma::mat const& scaP)
  {
  ...
  //! @brief Set list of model parameter scenaros and parameter scaling (vector
  //! format)
  void
  set_parameter(std::vector<FFVar> const& P,
                std::list<std::vector<double>> const& l_valP,
  ...
  void
  set_parameter(std::vector<FFVar> const& P,
                std::list<std::pair<std::vector<double>, double>> const& l_valP,
                arma::mat const& scaP)
  {
    BASE_MBFA::set_parameter(P, l_valP);
    _mPARSCA = scaP;
  ...
  void
  add_prior_campaign(std::list<std::pair<double, std::vector<double>>> const& C)
  {
  ...
  void
  set_prior_campaign(std::list<std::pair<double, std::vector<double>>> const& C)
  {
  ...
  static void effort_apportion(unsigned const n, unsigned const* typ,
                               double* val);
 ...
 private:
  ...
  void set_loglikelihood(FFVar const& LL)             = delete;
...
BASE_MBDOE::effort_apportion(unsigned const n, unsigned const* typ, double* val)
{
  ...
  for (double ratio = 1.; ratio > 0.1 && iround < 100; ++iround)
  {
    ...
    for (unsigned i = 0; i < n; ++i)
    {
      ...
      intval[i] =
          (val[i] < TOLZERO ? 0
                            : (supp <= sum ? std::ceil(ratio * val[i])
                                           : std::round(ratio * val[i])));
      intsum += intval[i];
    }
    if (sum < intsum)
      ratio *= 0.9;
...
#ifdef MAGNUS__EXPDES_SHOW_APPORTION
    if (val[i] > TOLZERO)
      std::cout << "X[" << i << "]: " << val[i] << std::endl;
#endif
  }
}

inline void
BASE_MBDOE::effort_rounding(unsigned const n, unsigned const* typ, double* val)
{
  double const TOLZERO = 1e-10;
  // double const TOLINT  = 1e-5;
  ...
  for (;;)
  {
    ...
    for (unsigned i = 0; i < n; ++i)
    {
      if (!typ[i]) continue;
      intsum += intval[i];
      // if( val[i] > TOLZERO ) std::cout << "X1[" << i << "]: " << intval[i] <<
      // std::endl;
    }
